...
function RegSubmit(ctrl) {
    $(ctrl).unbind("click");
    var arrTds = new Array("#dvUser", "#dvPwd","#dvRepwd", "#dvTelephone", "#dvEmail", "#dvCode", "#dvRec");
    BlurEmail();
    BlurUName();
    BlurPwd();
	BlurRec();
	...
	$.ajax({
		...
		data: {"txtUser": $.trim($("#txtUser").val()),"txtPwd": $.trim($("#txtPwd").val()),"txtTelephone":$.trim($("#txtTelephone").val()),"txtEmail": $.trim($("#txtEmail").val()),"sVerCode": $.trim($("#txtCode").val()),"txtRec": $.trim($("#txtRec").val())},
		//timeout: 8000,
		cache: false,
		type: "post",
		dataType: "json",
