       ...
       */
        ...
        */
        ...
        {  
            ...
            $transfer = M('borrow_investor i')
                ...
                ->join("{$this->pre}member_info m ON i.borrow_uid = m.uid")
                ->field("i.id, i.borrow_id, i.add_time, i.deadline, i.investor_interest, i.investor_capital, b.borrow_name, b.borrow_interest_rate, m.real_name")
            ...
            foreach($transfer as $k=>$v){
               ...
               if($arr['re_num']){//可转让债权，未还期数为0的就不需要展示了 by whh 2015-10-15
        ...
        */
		...
		public function sell($invest_id, $price)
        {
            ...
            if($check==='TRUE'){ // 检测通过
				...
				$info['debt_fee'] = round($price * $datag['debt_fee'] / 100, 2);
        ...
        */
		...
		private function checkSell($invest_id, $price)
        {
           ...
           // $total = M("investor_detail")->field("sum(capital+interest) as total_money ")->where($condition)->find();
            $total = M("investor_detail")->field("sum(capital) as total_money ")->where($condition)->find();
            if($price > $total['total_money']*1.1){ // 有没有超出价格上限
        ...
        */
        ...
        {  
            ...
            $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->join("{$this->pre}member_info m ON d.buy_uid=m.uid")
                ->field("d.invest_id, i.borrow_id, d.money, d.transfer_price, d.buy_time, d.status, m.real_name,  
        ...
        */
        public function buyDetb()
        {
            ...
            $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                ->join("{$this->pre}member_info m ON d.sell_uid=m.uid")
                ->join("{$this->pre}investor_detail td ON (d.invest_id = td.invest_id and d.buy_uid = td.investor_uid)")
                ->field("d.invest_id, i.borrow_id, (sum(td.interest) + d.money) as money, d.transfer_price, d.buy_time, d.status, d.serialid, m.real_name,  
                            d.cancel_times, d.period, d.total_period, b.borrow_name, b.borrow_interest_rate, b.total, b.has_pay")
        ...
        */
        public function onDetb()
        {
			...
			//---------------增加了real_name的信息 by whh 2015-10-08------------
			/*
			 $lists['data'] = M('invest_detb d')
                ->join("{$this->pre}borrow_investor i ON d.invest_id = i.id")
                ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
            ...
            $lists['data'] = M('invest_detb d')
                ...
                ->field("d.invest_id, i.borrow_id, d.money, d.serialid, m.user_name,  
                            d.total_period, b.borrow_name, b.total, b.has_pay,mi.real_name")
                ->where("d.buy_uid = ".$this->uid."  and d.status = 1 ")
                ->limit($Page->firstRow. ',' . $Page->listRows)
                ->order('d.id')
                ->select();
        ...
        */
        public function listAll($parm,$size=0, $pstatus=true)
        {
			...
			//$map['d.status']=array("neq","3");
			$map['d.status']=array("not in","3,99");
            ...
            $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,
                     i.investor_uid, i.deadline, b.id, b.borrow_name, b.borrow_interest_rate,b.borrow_status,b.borrow_type,
                     b.borrow_duration,m.credits, m.user_name";//sum(id.interest) as interest
            if($pstatus){ 
                ...
                $list['data'] = M("invest_detb d")
                        ...
                        ->where($map)
                        ->limit($Page->firstRow .','. $Page->listRows)
                        ->order("d.status desc,d.addtime asc")
                        ->select();
                        $sql = M()->getLastsql();
                $list['page'] = $Page->show();
               // $list['sql']=$sql;
                 ...
                 $field = "d.transfer_price, d.status, d.money, d.total_period, d.period, d.valid, d.id as debt_id, i.id as invest_id,
                     i.investor_uid, i.deadline, b.id, b.borrow_name, b.borrow_interest_rate,b.borrow_status,b.borrow_type,
                     b.borrow_duration,m.credits, m.user_name,id.interest";//
                 $map['id.status']=array("in","6,7");
                $list['data'] = M("invest_detb d")
                        ->join("{$this->pre}borrow_investor i ON d.invest_id=i.id")
                        ->join("{$this->pre}borrow_info b ON i.borrow_id = b.id")
                        ->join("{$this->pre}members m ON i.investor_uid=m.id")
                        ->join("{$this->pre}investor_detail id ON d.invest_id=id.invest_id")
                        ->field($field)
                        ->where($map)
                        ->limit($this->pagesize)
                        ->order("d.status desc,d.addtime asc")
        ...
        */
		...
		public function buy($invest_id)
        {
            ...
            if($check_result === 'TRUE'){
                ...
                $debt_interest = M("investor_detail")->field('sum(interest) as interest')->where("invest_id={$invest_id} and status in (6,7)")->find();//待收利息
				//file_put_contents('aaaa.php', M("investor_detail")->getlastsql());
                ...
                $buy_user['money_collect'] = $debt_info['money'] +$buy_user['money_collect'] + $debt_interest['interest'];//增加待收
                // 转让手续费扣除
                $datag = get_global_setting();
                $debt_fee = $datag['debt_fee'];// 百分比   1
                ...
                $sell_user['money_collect'] = $sell_user['money_collect'] - $debt_info['money']-$debt_interest['interest'];// 减去待收
				...
				//if($investor_status && $detail_status && $debt_status){
                    $this->Model->commit();
                    $this->moneyLog($this->uid, 46, -$debt_info['transfer_price'], $debt_info['money'], "购买{$serialid}号债权", $debt_info['sell_uid'],$debt_interest['interest']);
$this->moneyLog($debt_info['sell_uid'], 47, $debt_info['transfer_price'], $debt_info['money'], "转让{$serialid}号债权", $this->uid,$debt_interest['interest']);
        ...
        */
        private function moneyLog($uid, $type, $money, $debt_money, $info, $target_uid,$debt_interest)
        {
            ...
            $money_log = M("member_moneylog")
                            ...
                            ->find();
            if($money > 0){               
            $money_log['affect_money'] = $money +$debt_interest;
            }else{
                $money_log['affect_money'] = $money - $debt_interest;
            }
            $money_log['uid'] = $uid;
            ...
            if($money > 0){  // 增加资金
                ...
                $money_log['collect_money'] = $money_log['collect_money'] - $debt_money- $debt_interest;//待收资金减少债权金额
                $money_log['affect_money'] = $money_log['affect_money'] -$debt_money -$debt_interest;
                ...
                M("member_moneylog")->add($money_log);
                $money_log['collect_money'] = $money_log['collect_money']+ $debt_money + $debt_interest;//待收资金增加债权金额
                $money_log['affect_money'] = $debt_money + $debt_interest;
        ...
        */
		...
		private function checkBuy($invest_id)
        {
