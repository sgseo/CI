...
class MCommonAction extends Action
{
	...
	protected function _initialize(){
		...
		if($this->notneedlogin === true){
			if(session("u_id")){
				...
				if(!in_array(strtolower(ACTION_NAME),array("actlogout",'regsuccess','emailverify','verify','corpregister2'))) redirect(__APP__."/member/");
	...
	protected function _memberlogin($uid){
		...
		if(is_array($vo)){
			...
			$vos = M('member_info')->field("uid,real_name")->where("uid=".$up['uid'])->find();                     
                        if(!empty($vos)){
                           $_SESSION['real_name'] = $vos['real_name'];
                        }
