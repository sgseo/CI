...
class BorrowAction extends HCommonAction {
	...
	public function save(){
		...
		//相关的判断参数
		$borrow['borrow_type'] = intval(session(text($_POST['vkey'])));//5
		...
		if($borrow['repayment_type'] == 1){//按天还
			...
			$borrow['borrow_fee'] = getFloatValue($fee_rate*$borrow['borrow_money'],2);
			$borrow['manage_rate'] = $fee_rate;
		}else{
			...
			if($borrow['borrow_duration']>$fee_borrow_manage[3]&&is_numeric($fee_borrow_manage[3])){
				...
				$borrow['borrow_fee'] = getFloatValue($fee_rate_2*$borrow['borrow_money'],2);
				$borrow['manage_rate'] = $fee_rate_2;
			}else{
				$borrow['borrow_fee'] = getFloatValue($fee_rate_1*$borrow['borrow_money'],2);
				$borrow['manage_rate'] = $fee_rate_1;
			}
		}
		...
		if($newid) $this->success("借款发布成功，网站会尽快初审",__APP__."/member/index");
