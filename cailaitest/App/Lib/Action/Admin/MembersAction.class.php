...
class MembersAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		//123954 123702 124288 124826 123590 124783
		$map['m.iscorp'] = 0;
		//wusu登录特别限制 2015-11-18 @author lj
		// echo session('admin_user_name');
		// session('admin_user_name')=='wusu'

		if(session('admin_user_name')=='wusu')
		{
			$map['m.recommend_id']=array("in",'123954,123702,124288,124826,123590,124783');
		}
		//分页处理
	...
	*/
    ...
    {
    	//$trxid=$_POST['unfreezeno'];
     	$this->display();
    }

    public function userunfreeze()
    {
    	//$trxid='201510210002528553';
    	 $trxid=$_POST['unfreezeno'];
    	$bi=M('borrow_investor');
    	$bires=$bi->getFieldByFreezetrxid($trxid,'borrow_id,investor_uid');
    	//echo M()->getLastSql();
    	//dei;
    	$borrowid=$bires['borrow_id'];
    	//$investoruid=$bires['investor_uid'];
	//	$borrowid=1039;
    	import("ORG.Huifu.Huifu");

		$huifu = new Huifu();

		$hfres=$huifu->usrUnFreeze($trxid,$borrowid,$sort_order,$type);
		echo "解冻成功";
		//dump($hfres);
    }
	...
	public function corp()
	{
		...
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
		$field= 'm.id,m.user_phone,m.reg_time,m.user_name,m.customer_name,m.user_leve,m.time_limit,m.status,mi.real_name,mm.money_freeze,mm.money_collect,(mm.account_money+mm.back_money) account_money,m.user_email,m.recommend_id,m.is_borrow,m.is_vip';
		$list = M('members m')->field($field)->join("{$this->pre}member_money mm ON mm.uid=m.id")->join("{$this->pre}member_info mi ON mi.uid=m.id")->where($map)->limit($Lsql)->order('m.id DESC')->select();
	...
	public function guaredit() {
		...
		$guar = M("member_guarrate")->where("uid={$id}")->find();
		foreach($guar as $k=>$v)
		{
			$vo[$k] = $v;
		}
    ...
    public function doEdit() {
		...
		require "uc_client/client.php";
			$r = uc_user_edit($model->user_name,'', $model->user_pass,'',1);
			if($r < 0 )
			{
				$this->error("修改密码失败！");
			}
	...
	public function doguarEdit() {
		...
		*/
        ...
        //保存当前数据对象
        if ($result || $result2 || $result3 || $result4) { //保存成功
			$model->commit();
			alogs("Members",0,1,'成功执行了会员信息资料的修改操作！');//管理员操作日志
            //成功提示
             //$this->assign('jumpUrl', __URL__."/".session('listaction'));
	...
	public function _listFilter($list){
		...
		foreach($list as $key=>$v){
			if($v['recommend_id']<>0){
				$m['uid'] = $v['recommend_id'];
				$info = M('member_info')->field('real_name')->where($m)->select();
				$v['recommend_name'] = $info[0]['real_name'];
				unset($m,$info);
				//$v['recommend_name'] = M("member_info")->getFieldById($v['recommend_id'],"user_name");
			 }else{
