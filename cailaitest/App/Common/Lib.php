...
function MU($url,$type,$vars=array(),$domain=false){
	//获得基础地址START
	$path = explode("/",trim($url,"/"));   //Array ( [0] => Home [1] => gonggao [2] => index )       
...
function get_type_infos(){
	...
	if(!isset($_GET['typeid'])){
		...
		$rurl = explode("?",$_SERVER['REQUEST_URI']); //Array ( [0] => /cfkt/index.html ) 
		$xurl_tmp = explode("/",str_replace(array("index.html",".html"),array('',''),$rurl[0]));//获得组合的type_nid   Array ( [0] => [1] => cfkt [2] => ) 
		$zu = implode("-",array_filter($xurl_tmp));//组合  cfkt
		//print_r($type_nid);
		$typeid = $type_nid[$zu];
...
function Notice($type,$uid,$data=array()){
	...
	switch($type){
		...
		case 8://验证中心邮件验证
			$vcode = rand_string($uid,32,0,1);
			$link='<a href="'.C('WEB_URL').'/member/common/emailverify?vcode='.$vcode.'">点击链接验证邮件</a>';
			$link='<a href="https://www.cailai.com/member/common/emailverify?vcode='.$vcode.'">点击链接验证邮件</a>';
...
//function investMoney($uid用户UID 即投资人,$borrow_id借款人,$money交易金额,$invest_info_id投标订单id){
function investMoney($uid,$borrow_id,$money,$_is_auto=0,$invest_info_id,$reqext='0'){
    ...
    $stmt1->execute();
	/////////////////////////////锁表  辉 2013-11-16////////////////////////////////////////////////
	$binfo = M("borrow_info")->field("borrow_uid,borrow_money,borrow_interest_rate,borrow_type,borrow_duration,repayment_type,has_borrow,reward_money,money_collect,manage_rate,danbao")->find($borrow_id);
	//file_put_contents('C:\www\qianhetou\App\Runtime\asss.log', M()->getLastSql());
	//新加入了奖金reward_money到资金总额里
	$vminfo = getMinfo($uid,'m.user_leve,m.time_limit,mm.account_money,mm.back_money,mm.money_collect');
	file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."\$money1020-1761:".print_r($money,true)."\n",FILE_APPEND);//450
	$temp=$vminfo['account_money']+$vminfo['back_money']+$binfo['reward_money'];
	file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."\$temp-1020-1761:".print_r($temp,true)."\n",FILE_APPEND);//450

	if(($vminfo['account_money']+$vminfo['back_money']+$binfo['reward_money'])<($money-$reqext)) {
	...
	$investMoney->startTrans();
		...
		$fs = FS("Webconfig/leveinvestconfig");
	...
	if($invest_defail_id && $invest_info_id && $upborrow_res){//还款概要和详情投标进度都保存成功
		$investMoney->commit();
			$temp1020=-($money-$reqext);
			file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."\$temp1020-2012:".print_r($temp1020,true)."\n",FILE_APPEND);//450
...
//				$huifu = new Huifu();			
//				$huifu->transfer($jiangli,$recommend_usrid['usrid']);
                                memberAward($uid,$vo['recommend_id'],$borrow_id,$investinfo['investor_capital'],$jiangli);//添加推荐奖励记录  zxm 2015-10-13
...
function borrowRefuse($borrow_id,$type){//$type=2 代表流标返还; $type=3代表复审未通过，返还
	...
	$investorList = M("borrow_investor i")->field('i.id,i.investor_uid,i.investor_capital,m.usrid,i.freezetrxid')->join("{$pre}members m on m.id = i.investor_uid" )->where("borrow_id={$borrow_id}")->select();
	//exit(M("borrow_investor")->getlastsql());
	...
	if(is_array($investorList)){
		...
		foreach($investorList as $v){
			...
			if($moneynewid_x){ 
				...
				$huifu->usrUnFreeze($v['freezetrxid']);
			}
...
function borrowApproved($borrowid)
{
	...
	foreach($investinfo as $v)
	{
		...
		$i++;
               // $msg='尊敬的财来网投资人'.$investor_member['real_name'].','.$total['borrow_name'].'已投标完成，你借给'.$borrow_member['real_name'].'的'.$v['investor_capital'].'元资金已划拨，'.$today.'开始计息。详情请咨询客户经理，或致电400-079-8558';
		$msg='尊敬的财来网投资人'.$investor_member['real_name'].','.$borrowid.'号标已投标完成，你借给'.$borrow_member['real_name'].'的'.$v['investor_capital'].'元资金已划拨，'.$today.'开始计息。详情请咨询客户经理，或致电400-079-8558';
		sendsms($v['user_name'], $msg);
                
                $data=array(
                    "member_id"=>$v['investor_uid'],
                    "content"=>$msg,
                );
...
// 添加推荐奖励记录  zxm 2015-10-13
function memberAward($uid,$recommendId,$borrowId,$tenderMoney,$awardMoney){
    $map=array(
       'uid'=>$uid, 
       'recommend_id'=>$recommendId, 
       'bid'=>$borrowId, 
       'tender_money'=>$tenderMoney,//投标金额
       'award_money'=>$awardMoney,//奖励
       'ordid'=>ordid(),//生产ordid
       'addtime'=>time(),
    );
    file_put_contents("/tmp/awardlog", date('m-d H:i:s')." return  ".print_r($map,true)."\n",FILE_APPEND);
    $M = D("member_award")->add($map);
}

//生成订单号和订单日期 zxm 2015-10-13
function ordid(){
        $nowtime = time();
        $ordid = date("YmdHis",$nowtime).rand(1000,9999);
        return $ordid;
}

function memberMoneyLog($uid,$type,$amoney,$info="",$target_uid="",$target_uname="",$fee=0,$reqext){
	$xva = floatval($amoney);
	file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."amoney1020-3139:".print_r($xva,true)."\n",FILE_APPEND);//450
	...
	$Moneylog->startTrans();
		...
		/////////////////////////////////////////
			...
			case 6://投标冻结
			case 37://投企业直投冻结
				$data['affect_money'] = $amoney;//450

				if(($MM['back_money']+$amoney+$fee)<0){//提现手续费先从回款余额资金池里扣，不够再去充值资金池里减少
					$data['back_money'] = 0;
					$data['account_money'] = $MM['account_money']+$MM['back_money']+$amoney+$fee;
					file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."\$data['account_money']-3217".print_r($data['account_money'],true)."\n",FILE_APPEND);//0.00
				}else{
					$data['back_money'] = $MM['back_money']+$amoney+$fee;
					$data['account_money'] = $MM['account_money'];
					file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."\$data['account_money']-3221:".print_r($data['account_money'],true)."\n",FILE_APPEND);//0.00
				}
		...
		////////////////////////////////////////
			...
			break;
			case 15:
			//投标成功冻结资金转为待收资金(要加上代金券的)
			case 39://企业直投投标成功冻结资金转为待收资金
				$data['affect_money'] = $amoney;
				$data['account_money'] = $MM['account_money'];
				$data['collect_money'] = $MM['money_collect']+$amoney+$reqext;//(要加上代金券的)
				$data['freeze_money'] = $MM['money_freeze']-$amoney;
				$data['back_money'] = $MM['back_money'];
			break;
			case 28://投标成功利息待收
		...
		$slj=M()->getlastsql();
		file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."rlj10-20:".print_r($slj,true)."\n",FILE_APPEND);
		//帐户更新
		$mmoney['money_freeze']=$data['freeze_money'];
		$mmoney['money_collect']=$data['collect_money'];
		file_put_contents('/tmp/debugwhh',date('m-d H:i:s')."rmmczhgx-3359".print_r($mmoney['money_collect'],true)."\n",FILE_APPEND);//0.00
...
function getTypeListActa($parm){
	...
	if($pc>0){
		...
		$map['is_hiden'] = 0;
		$map['parent_id'] = D('Acategory')->getFieldById($parm['type_id'],'parent_id');    //SELECT `parent_id` FROM `lzh_article_category` WHERE ( `id` = '37' ) LIMIT 1   zw
		//$data = D('Acategory')->field($field)->where($map)->order($Osql)->limit($Lsql)->select();
		$data = D('Acategory')->field($field)->where($map)->order($Osql)->limit($Lsql)->select(); //获得所有父类 zw
	}
	//链接处理
	$typefix = get_type_leve_nid($parm['type_id']); //type_nid
	$typeu = $typefix[0]; //本身type_nid
	$suffix=C("URL_HTML_SUFFIX");
	foreach($data as $key=>$v){
		if($v['type_set']==2){
			if(empty($v['type_url'])) $data[$key]['turl']="javascript:alert('请在后台添加此栏目链接');";
...
function getMemberBorrowScan($uid){
	...
	$repaymentNum=M('investor_detail')->field($field)->where("borrow_uid = {$uid}")->group('sort_order,borrow_id')->select();
         file_put_contents('/tmp/borrowNum',date('m-d H:i:s')." borrowNum : ".print_r(M('investor_detail')->getLastSql(),true)."\n",FILE_APPEND);
	foreach($repaymentNum as $v){
        ...
        file_put_contents('/tmp/borrowNum',date('m-d H:i:s')." borrowNum : ".print_r($repaymentStatus,true)."\n",FILE_APPEND);
	//还款情况相关
	//借出情况相关
...
/*function getUserWC($uid){
	$row=array();
	$field="count(id) as num,sum(transamt) as money";
	$row["W"] = M('member_withdrawlog')->field($field)->where("uid={$uid} AND status=1")->find();
	$field="count(id) as num,sum(affect_money) as money";
	$row["C"] = M('member_moneylog')->field($field)->where("uid={$uid} AND type=29")->find();
	return $row;
}*/
function getUserWC($uid){
	$row=array();
	...
	$row["W"] = M('member_withdrawlog')->field($field)->where("uid={$uid} AND status=1")->find();
	$field="count(id) as num,sum(transamt) as money";
	$row["C"] = M('member_paylog')->field($field)->where("uid={$uid}")->find();
	return $row;
}
...
function getNet($minfo,$capitalinfo){
	...
	return getFloatValue($minfo['money_collect'],2);	
}
		...
		if($invest_info_id && $Tinvest_defail_id){
			...
			if($investinfo['reward_money']>0){
				...
				if($_reward_m && $_reward_m_give) 
				{
					...
					/////////////////////////////////////////////////////汇付托管 2014-10-09///////////////////////////////////////////////////////////////
					/*******************************************************商户奖励************************************************************************/
					$transamt = $investinfo['reward_money'];
					$info1 = M("huifulog")->distinct(true)->field("usrid")->where("uid=".$uid)->find();
					$usrid = $info1['usrid'];
					
					import("ORG.Huifu.Huifu");
					$huifu = new Huifu();
					$huifu->transfer($transamt,$usrid);
				}
			}
			//投标奖励
			//file_put_contents("D:\wwwroot\App\Runtime\a.txt","dsdsda");	
			//////////////////////邀请奖励开始////////////////////////////////////////
			...
			if($vo['recommend_id']!=0){
				import("ORG.Huifu.Huifu");
				$huifu = new Huifu();
				$dom = $huifu->transfer($jiangli,$vo['recommend_id']);
				if($dom){
					memberMoneyLog($vo['recommend_id'],13,$jiangli,$vo['user_name']."对{$borrow_id}号标投资成功，你获得推广奖励".$jiangli."元。",$uid);
				}
			...
			////////////////////////////////////////回款续投奖励规则 fan 2013-07-20////////////////////////////
				...
				/////////////////////////////////////////////////////汇付托管 2014-10-09///////////////////////////////////////////////////////////////
				$info1 = M("huifulog")->distinct(true)->field("usrid")->where("uid=".$uid)->find();
				$info2 = M("huifulog h")->distinct(true)->field("h.usrid")->join("{$pre}transfer_borrow_info s ON s.borrow_uid=h.uid")->where("s.id=".$borrow_id)->find();
				$usrid = $info1['usrid'];
				$borrowerid = $info2['usrid'];
				$transamt = $money;
				$type = "Q";//表示企业标
				$investid = $invest_info_id;
				
				import("ORG.Huifu.Huifu");
				$huifu = new Huifu();
				$huifu->initiativeTender($usrid,$transamt,$borrowerid,$type,$investid);
				/////////////////////////////////////////////////////汇付托管 END///////////////////////////////////////////////////////////////
			}
		}else{
			$investMoney->rollback();
...
// 返回 1:企业担保 2:企业用户 3:个人用户
function member_type($uid)
{
	$find = M('members')->field('isdanbao,iscorp')->where('id='.$uid)->find();
	$type = array();
	if($find['isdanbao'] == 1 && $find['iscorp'] == 1)
	{
		$type = array("status"=>1,"type"=>"担保");
		return $type;
	}else if($find['isdanbao'] == 0 && $find['iscorp'] == 1)
	{
		$type = array("status"=>2,"type"=>"企业");
		return $type;
	}else if($find['isdanbao'] == 0 && $find['iscorp'] == 0)
	{
		$type = array("status"=>3,"type"=>"个人");
		return $type;
	}else{
	$type = array("status"=>0,"type"=>"黑户");
		return $type;
	}
}
?>
