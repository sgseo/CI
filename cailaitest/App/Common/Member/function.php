...
function getWithDrawLog($map,$size,$limit=10){
	...
	if($size){
		//分页处理
		import("ORG.Util.Page");
		$count = M('member_withdrawlog')->where($map)->count('id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	...
	$status_arr =array('待审核','已提现','处理中','提现失败');
	$list = M('member_withdrawlog')->where($map)->order('id DESC')->limit($Lsql)->select();
	//dump($list);exit;
...
function getChargeLog($map,$size,$limit=10){
	...
	if($size){
		...
		$count = M('member_paylog')->where($map)->count('id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	...
	$list = M('member_paylog')->where($map)->order('id DESC')->limit($Lsql)->select();
	foreach($list as $key=>$v){
		$list[$key]['status'] = $status_arr[$v['status']];
	}
	...
	$row['list'] = $list;
	$row['page'] = $page;
	$row['total_money'] = M('member_paylog')->where($map)->sum('transamt');
	return $row;
}
//借款逾期但还未还的借款列表(逾期)
...
function getBorrowList($map,$size,$limit=10){
	...
	$Model = D("BorrowView");
	$list=$Model->field(true)->where($map)->order('borrow_money-has_borrow DESC ,id DESC')->group('id')->limit($Lsql)->select();
...
function getBorrowList2($map,$size,$limit=10){
	//if(empty($map['borrow_uid'])) return;
	...
	/////////////使用了视图查询操作 fans 2013-05-22/////////////////////////////////
	$Model = D("BorrowView");
            $list=$Model->field(true)->where($map)->order('times DESC')->group('id')->limit($Lsql)->select();
	//echo M()->getlastsql();exit;
...
function getTenderList($map,$size,$limit=10){
	...
	if($size){
		//分页处理
		import("ORG.Util.Page");
		$count = M('borrow_investor i')->join("{$pre}borrow_info Borrow ON Borrow.id=i.borrow_id")->where($map)->count('i.id');
		$p = new Page($count, $size);
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
	...
	$list=$Model->field(true)->where($map)->order('times ASC')->group('id')->limit($Lsql)->select();
        ...
        file_put_contents('/tmp/tend',date('m-d H:i:s')." back : ".print_r($Model->getLastSql(),true)."\n",FILE_APPEND);
	////////////////////////视图查询 fan 20130522//////////////////////////////////////////
	//dump($list);die;
	foreach($list as $key=>$v){
		//if($map['i.status']==4){
	...
	$row['list'] = $list;
	$row['page'] = $page;
	$row['total_money'] = M('borrow_investor i')->join("{$pre}borrow_info Borrow ON Borrow.id=i.borrow_id")->where($map)->sum('investor_capital');
	$row['total_num'] = $count;
	return $row;
}
...
function getMVouchList($map,$size){
	...
	foreach($list as $key=>$v){
		$list[$key]['borrow_progress'] = ceil($v['has_borrow']/$v['borrow_money'])*100;
		$list[$key]['vouch_progress'] = ceil($v['has_vouch']/$v['borrow_money'])*100;
