<?php
    ...
    */
	class Huifu
	{
		...
		const USER_UNFREEZE = "/home/returl/unfreeze";//资金（货款）解冻回调地址
		const USER_UNFREEZE_BACK ="/home/returl/usrunfreeze";//资金（货款）解冻后台回调地址
		//const USER_NEW_UNFREEZE="/home/returl/newbie_usrres";
		//const USER_NEW_UNFREEZE_BACK="/home/returl/newbie_usrunfreeze";
		...
		const REPAYMENT_NEW_BACK = "/home/returl/newbie_repayment";//新手标自动扣款 （还款）后台回调 2015-09-16 lj
...
//			$usrCustId = $usrid;//用户客户号
		...
		*/
		public function usrUnFreeze($trxid,$borrowid,$sort_order,$type)
		{
			...
			$info['type'] = $type;//解冻类型
			//$info['investoruid']=$investoruid;
			$merPriv = base64_encode(json_encode($info));
			$res=$this->chinapnr->usrUnFreeze($merCustId,$ordId,$ordDate,$trxId,$retUrl,$bgRetUrl,$merPriv);
			return $res;//nul
		}
		//新手标解冻资金
		/*
		...
		**$type 解冻类型（1：投标解冻 2：保证金解冻）
		*/
			public function newbie_usrUnFreeze($trxid,$borrowid,$borrow_uid,$type)
		{
			$ord = $this->ord();
			$merCustId = $this->merCustId;//商户客户号
			$ordId = $ord['id'];//订单号
			$ordDate = $ord['date'];//订单日期
			$trxId = $trxid;//交易流水号
			$retUrl = Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_NEW_UNFREEZE;//资金（货款）解冻回调地址
			$bgRetUrl = Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_NEW_UNFREEZE_BACK;//资金（货款）解冻后台回调地址
			$info = array();
			$info['borrowid'] = $borrowid;//标ID
			$info['borrow_uid'] = $borrow_uid;//借款人是谁
			$info['type'] = $type;//解冻类型
			$merPriv = base64_encode(json_encode($info));//商户自定义参数 返回时 原路返回
		...
		**$suborddate 关联投标订单日期
		**$borrowid 标ID
		**$sort_order 期数（针对分期还款，表示当前期数）2015-09-16 lj 新手标无期数故取消
		**repaymentid 还款ID
		**$investorid 投资人ID
		**$type 还款类型（1：会员自己还，2：网站代还，3：担保公司代还）
		*/
		public function repayment($usrid,$transamt,$Fee,$borrowercustid,$divdetails,$subordid,$suborddate,$borrowid,$repaymentid,$investorid,$type)
		{
			$ord = $this->ord();
			$merCustId = $this->merCustId;//商户客户号
			$ordId = $ord['id'];//订单号
			...
			$feeObjFlag = "I";//手续费收取对象标志（I：向入款客户号InCustId收取）
			$bgRetUrl = Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::REPAYMENT_NEW_BACK;//自动扣款（还款）新手标 后台回调地址 2015-09-16 lj
			$info = array();
			$info['borrowid'] = $borrowid;//标ID
		...
		**$usrid 收款人客户号
		*/
		public function transfer($transamt,$usrid,$ordId='')
		{
                        if(empty($ordId)){
                            $ord = $this->ord();
                            $ordId = $ord['id'];//订单号
