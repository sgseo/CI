...
class Chinapnr{
    ...
    private function sign_npc($msg)
    {
        //$npc_server_host = "10.162.83.124";
        $npc_server_host = "115.29.245.207";
        $npc_server_port = "40000";
        $npc_server_type_sign = "S";
        $npc_server_type_ver = "V";
        $npc_server_id = "530466";

        $port = $npc_server_port;
        
		$socket = socket_create(AF_INET,SOCK_STREAM,SOL_TCP) or die("Could not create socket\n");
        
		$msg = sprintf("%04d",strlen($msg)).$msg;
        $msg = $npc_server_type_sign.$npc_server_id.$msg;
        $msg = sprintf("%04d",strlen($msg)-1).$msg;
        
		$connection = socket_connect($socket,$npc_server_host,$port);
        
		socket_write($socket,$msg,strlen($msg)) or die("Write failed\n");
        
		$auth_str = '';
        $auth_str = socket_read($socket,267,PHP_NORMAL_READ);
        
		socket_close($socket);
        
		$auth_str = substr($auth_str,11);
	 ...
	 * @return mixed
	 */
	private function request($postData=array()){//echo "<pre>";print_r($postData);echo "</pre>";exit;
	...
	private function request_multi($postData=array())
	{
		$ch = array();
		$options = array(CURLOPT_URL=>$this->platformUrl,
						CURLOPT_HEADER=>0,
		...
		{
			$ch[$k] = curl_init();
			$post_string = $this->postArrayToString($v);
	...
	private function reactResponse($res= "", $signKeys=array()){
		...
		if($ret){
			if($this->verify($this->getSignContent($ret, $signKeys), urldecode($ret['ChkValue'])))
				return $ret;			
		}		
		return null;
	}
	 ...
	 * @return no return
	 */
	private function autoRedirect($reqData= array()){//echo "<pre>";print_r($reqData);echo "</pre>";exit;
		$tmp = array(); 
		$html= <<<HTML
<html><head>
...
HTML;
		...
		//其中tmp里面包含 回调的url 新手标initiativeTender 新手标后台ininewbietenderreturn
		print $html;
		exit;
	}
	...
	public function userRegister($merCustId, $bgRetUrl, $retUrl="", $usrId="", $usrName="", $idType="", $idNo="", $usrMp="", $usrEmail="", $merPriv="", $charSet=""){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USER_REGISTER.$merCustId.$bgRetUrl.$retUrl.$usrId.$usrName.$idType.$idNo.$usrMp.$usrEmail.$merPriv);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_USER_REGISTER,
				"MerCustId"	=>	$merCustId,
				"BgRetUrl"	=>	$bgRetUrl,
				"RetUrl"	=>	$retUrl,
				"UsrId"		=>	$usrId,
	...
	public function queryBalance($merCustId,$usrCustId){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_BALANCE.$merCustId.$usrCustId);
		$reqData= array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_BALANCE,
			"MerCustId"	=>	$merCustId,
			"UsrCustId" =>	$usrCustId,
			"ChkValue"	=>	$checkValue,
		);
	...
	public function queryAccts($merCustId){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_ACCTS.$merCustId);
		echo '$checkValue:';
		print_r($checkValue);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_ACCTS,
			"MerCustId"	=>	$merCustId,
			"ChkValue"	=>	$checkValue,
		);
		echo '$reqData:';
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_RETURN_DZ_FEE.$merCustId.$beginDate.$endDate);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_RETURN_DZ_FEE,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"ChkValue"	=>	$checkValue,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_TRANS_STAT.$merCustId.$ordId.$ordDate.$queryTransType);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_TRANS_STAT,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"OrdDate"	=>	$ordDate,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize.$queryTransType);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CASH_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_CASH_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_BALANCE_BG.$merCustId.$usrCustId);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_BALANCE_BG,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"ChkValue"	=>	$checkValue,
		);
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_TENDER_PLAN.$merCustId.$usrCustId);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_TENDER_PLAN,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"ChkValue"	=>	$checkValue,
		);
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_TRF_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_TRF_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_SAVE_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_SAVE_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CORP_REGISTER_QUERY.$merCustId.$busiCode.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_CORP_REGISTER_QUERY,
			"MerCustId"	=>	$merCustId,
			"BusiCode"	=>	$busiCode,
			"ReqExt"	=>	$reqExt,
			"ChkValue"	=>	$checkValue,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CREDIT_ASSIGN_RECONCILIATION.$merCustId.$ordId.$beginDate.$endDate.$sellCustId.$buyCustId.$pageNum.$pageSize.$reqExt);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_CREDIT_ASSIGN_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_FSS_PURCHASE_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize.$reqExt);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_FSS_PURCHASE_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_FSS_REDEEM_RECONCILIATION.$merCustId.$beginDate.$endDate.$pageNum.$pageSize.$reqExt);
		
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_FSS_REDEEM_RECONCILIATION,
			"MerCustId"	=>	$merCustId,
			"BeginDate"	=>	$beginDate,
			"EndDate"	=>	$endDate,
			"PageNum"	=>	$pageNum,
	...
	public function queryFss($merCustId,$reqExt){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_FSS.$merCustId.$reqExt);
		$reqData= array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_FSS,
			"MerCustId"	=>	$merCustId,
			"ReqExt" =>	$reqExt,
			"ChkValue"	=>	$checkValue,
		);
	...
	public function queryFssAccts($merCustId,$usrCustId,$reqExt){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_FSS_ACCTS.$merCustId.$usrCustId.$reqExt);
		$reqData= array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_FSS_ACCTS,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"ReqExt" =>	$reqExt,
			"ChkValue"	=>	$checkValue,
	...
	public function queryCardInfo($merCustId,$usrCustId,$cardId){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_CARD_INFO.$merCustId.$usrCustId.$cardId);
		$reqData= array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_CARD_INFO,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"CardId" =>	$cardId,
			"ChkValue"	=>	$checkValue,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USER_LOGIN.$merCustId.$usrCustId);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_USER_LOGIN,
				"MerCustId"	=>	$merCustId,
				"UsrCustId"	=>	$usrCustId,
				"ChkValue"	=>	$checkValue,
		);//echo "<pre>";print_r($reqData);echo "</pre>";exit;
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_ACCT_MODIFY.$merCustId.$usrCustId);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_ACCT_MODIFY,
				"MerCustId"	=>	$merCustId,
				"UsrCustId"	=>	$usrCustId,
				"ChkValue"	=>	$checkValue,
		);
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_AUTO_TENDER_PLAN.$merCustId.$usrCustId.$tenderPlanType.$transAmt.$retUrl.$merPriv);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_AUTO_TENDER_PLAN,
				"MerCustId"	=>	$merCustId,
				"UsrCustId"	=>	$usrCustId,
		...
		//print_r($reqData);
		//echo "</pre>";exit;
//                file_put_contents('/tmp/zwerror',date('m-d H:i:s')."autoTenderPlan".print_r($reqData,true)."\n",FILE_APPEND);
		$this->autoRedirect($reqData);
	}
	/**
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CORP_REGISTER.$merCustId.$usrId.$usrName.$instuCode.$busiCode.$taxCode.$merPriv.$guarType.$bgRetUrl.$reqExt);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_CORP_REGISTER,
				"MerCustId"	=>	$merCustId,
				"UsrId"	=>	$usrId,
				"UsrName"	=>	$usrName,
				"InstuCode"	=>	$instuCode,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_AUTO_TENDER_PLAN_CLOSE.$merCustId.$usrCustId.$retUrl.$merPriv);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_AUTO_TENDER_PLAN_CLOSE,
				"MerCustId"	=>	$merCustId,
				"UsrCustId"	=>	$usrCustId,
				"RetUrl"	=>	$retUrl,
				"MerPriv"	=>	$merPriv,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_NET_SAVE.$merCustId.$usrCustId.$ordId.$ordDate.$gateBusiId.$openBankId.$dcFlag.$transAmt.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_10,
				"CmdId"		=>	Chinapnr::CMDID_NET_SAVE,
				"MerCustId"	=>	$merCustId,
				"UsrCustId"	=>	$usrCustId,
				"OrdId"	=>	$ordId,
				"OrdDate"	=>	$ordDate,
				...
				"ChkValue"	=>	$checkValue,
                 ...
                 file_put_contents('/tmp/zw2.txt',date('m-d H:i:s')."reqData ".print_r($reqData,true)."\n",FILE_APPEND); 
		$this->autoRedirect($reqData);
	}
	/**
	...
	public function initiativeTender($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$maxTenderRate,$borrowerDetails,$isFreeze,$freezeOrdId = '',$retUrl = '',$bgRetUrl,$merPriv,$reqExt = '')
	{
		...
		$reqExt = $this->arrayToJsonStr($reqExt);//json 字符串
		}
		$str = Chinapnr::VERSION_20.Chinapnr::CMDID_INITIATIVE_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$isFreeze.$freezeOrdId.$retUrl.$bgRetUrl.$merPriv.$reqExt;
		Log::write("initiative_".$str);
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_INITIATIVE_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$isFreeze.$freezeOrdId.$retUrl.$bgRetUrl.$merPriv.$reqExt);
		Log::write("initiative_sign_".$checkValue);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_20,
				"CmdId"		=>	Chinapnr::CMDID_INITIATIVE_TENDER,
				"MerCustId"	=>	$merCustId,
				"OrdId"	=>	$ordId,
				"OrdDate"	=>	$ordDate,
				"TransAmt"	=>	$transAmt,
		...
		if($reqExt)
		{
			$reqData["ReqExt"]=$reqExt;
		}
		file_put_contents('/tmp/isnew2',date('m-d H:i:s')."201请求new: ".print_r($reqData,true)."\n",FILE_APPEND);
		Log::write("initiativeTender_".json_encode($reqData));
		// echo "<pre/>";var_dump($reqData);
		// die;
		$this->autoRedirect($reqData);
	}
	/**
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_TENDER_CANCLE.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$isUnFreeze.$unFreezeOrdId.$freezeTrxId.$retUrl.$bgRetURL.$merPriv.$reqExt);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_20,
				"CmdId"		=>	Chinapnr::CMDID_TENDER_CANCLE,
				"MerCustId"	=>	$merCustId,
				"OrdId"	=>	$ordId,
				"OrdDate"	=>	$ordDate,
				"TransAmt"	=>	$transAmt,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USER_BIND_CARD.$merCustId.$usrCustId.$bgRetURL.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_USER_BIND_CARD,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"BgRetUrl"	=>	$bgRetURL,
			"MerPriv"	=>	$merPriv,
	 ...
	 * @return Ambigous <multitype:, NULL, mixed>
	 */
	public function cash($merCustId,$ordId,$usrCustId,$transAmt,$servFee = '',$servFeeAcctId = '',$openAcctId = '',$retUrl = '',$bgRetURL,$remark = '',$charSet = '',$merPriv = '',$reqExt = '', $cashChl='')
	{
		// $yuan=Chinapnr::VERSION_20.Chinapnr::CMDID_CASH.$merCustId.$ordId.$usrCustId.$transAmt.$servFee.$servFeeAcctId.$openAcctId.$retUrl.$bgRetURL.$remark.$merPriv.$reqExt.$cashChl;
		// 	file_put_contents("/tmp/a.txt", print_R($yuan,true));

		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_CASH.$merCustId.$ordId.$usrCustId.$transAmt.$servFee.$servFeeAcctId.$openAcctId.$retUrl.$bgRetURL.$remark.$merPriv.$reqExt);

		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_20,
			"CmdId"		=>	Chinapnr::CMDID_CASH,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"UsrCustId"	=>	$usrCustId,
			"TransAmt"	=>	$transAmt,
		...
		//echo "<pre>";print_r($reqData);echo "</pre>";exit;
		file_put_contents("/tmp/a.txt", print_R($reqData,true));
		$this->autoRedirect($reqData);		
	}	
	/**
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USR_ACCT_PAY.$ordId.$usrCustId.$merCustId.$transAmt.$inAcctId.$inAcctType.$retUrl.$bgRetURL.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_USR_ACCT_PAY,
			"OrdId"	=>	$ordId,
			"UsrCustId"	=>	$usrCustId,
			"MerCustId"	=>	$merCustId,
			"TransAmt"	=>	$transAmt,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USR_TRANSFER.$ordId.$usrCustId.$merCustId.$transAmt.$inUsrCustId.$retUrl.$bgRetURL.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_USR_TRANSFER,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"RetUrl"	=>	$retUrl,
			"BgRetUrl"	=>	$bgRetURL,
	...
	public function creditAssign($merCustId,$sellCustId,$creditAmt,$creditDealAmt,$bidDetails,$fee,$divDetails = '',$buyCustId,$ordId,$ordDate,$retUrl = '',$bgRetURL,$merPriv = '',$reqExt = '')
	{
		...
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CREDIT_ASSIGN.$merCustId.$sellCustId.$creditAmt.$creditDealAmt.$bidDetails.$fee.$divDetails.$buyCustId.$ordId.$ordDate.$retUrl.$bgRetURL.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_CREDIT_ASSIGN,
			"MerCustId"	=>	$merCustId,
			"SellCustId"	=>	$sellCustId,
			"CreditAmt"	=>	$creditAmt,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_FSS_TRANS.$merCustId.$usrCustId.$ordId.$ordDate.$retUrl.$bgRetURL.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_FSS_TRANS,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"OrdId"	=>	$ordId,
			"OrdDate"	=>	$ordDate,
	...
	public function queryAcctDetails($merCustId,$usrCustId){
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_QUERY_ACCT_DETAILS.$merCustId.$usrCustId);
		$reqData= array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_QUERY_ACCT_DETAILS,
			"MerCustId"	=>	$merCustId,
			"UsrCustId" =>	$usrCustId,
			"ChkValue"	=>	$checkValue,
		);
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_BG_BIND_CARD.$merCustId.$usrCustId.$openAcctId.$openBankId.$openProvId.$openAreaId.$openBranchName.$isDefault.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_BG_BIND_CARD,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"OpenAcctId"	=>	$openAcctId,
			"OpenBankId"	=>	$openBankId,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_DEL_CARD.$merCustId.$usrCustId.$cardId);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_DEL_CARD,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"CardId"	=>	$cardId,
			"ChkValue"	=>	$checkValue,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_POS_WH_SAVE.$merCustId.$usrCustId.$openAcctId.$transAmt.$ordId.$ordDate.$checkDate.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_POS_WH_SAVE,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
			"OpenAcctId"	=>	$openAcctId,
			"TransAmt"	=>	$transAmt,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USR_FREEZE_BG.$merCustId.$usrCustId.$subAcctType.$subAcctId.$ordId.$ordDate.$transAmt.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_USR_FREEZE_BG,
			"MerCustId"	=>	$merCustId,
			"UsrCustId"	=>	$usrCustId,
	...
	{
		//Chinapnr::VERSION_10.Chinapnr::CMDID_USR_UN_FREEZE.$merCustId.$ordId.$ordDate.$trxId.$retUrl.$bgRetUrl.$merPriv
		//return $retUrl;
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_USR_UN_FREEZE.$merCustId.$ordId.$ordDate.$trxId.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_USR_UN_FREEZE,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"OrdDate"	=>	$ordDate,
			"TrxId"	=>	$trxId,
	...
	public function autoTender($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$maxTenderRate,$borrowerDetails,$isFreeze,$freezeOrdId = '',$retUrl = '',$bgRetUrl,$merPriv,$reqExt = '')
	{
		$borrowerDetails = $this->arrayToJsonStr($borrowerDetails);
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_AUTO_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$isFreeze.$freezeOrdId.$retUrl.$bgRetUrl.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_20,
			"CmdId"		=>	Chinapnr::CMDID_AUTO_TENDER,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"OrdDate"	=>	$ordDate,
			"TransAmt"	=>	$transAmt,
		...
		$response = $this->reactResponse($this->request($reqData),array("CmdId","RespCode","RespDesc","MerCustId","OrdId","OrdDate","TransAmt","UsrCustId","TrxId","IsFreeze","FreezeOrdId","FreezeTrxId","RetUrl","BgRetUrl","MerPriv"));
		return $response;
	}
	public function autoTender_multi($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$maxTenderRate,$borrowerDetails,$isFreeze,$freezeOrdId = '',$retUrl = '',$bgRetUrl,$merPriv,$reqExt = '')
	{
		$chinapnrstart = microtime(true);
		$checkValue = array();
		$reqData = array();
		foreach($usrCustId as $k=>$v)
		{
			$borrowerDetails[$k] = $this->arrayToJsonStr($borrowerDetails[$k]);
			$signstart = microtime(true);
			$checkValue[$k] = $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_AUTO_TENDER.$merCustId.$ordId[$k].$ordDate[$k].$transAmt[$k].$usrCustId[$k].$maxTenderRate.$borrowerDetails[$k].$isFreeze[$k].$freezeOrdId[$k].$retUrl.$bgRetUrl.$merPriv.$reqExt);
			...
			"OrdDate"	=>	$ordDate[$k],
			"TransAmt"	=>	$transAmt[$k],
			"UsrCustId"	=>	$usrCustId[$k],
			"MaxTenderRate"	=>	$maxTenderRate,
			"BorrowerDetails"	=>	$borrowerDetails[$k],
			"IsFreeze"	=>	$isFreeze[$k],
			"FreezeOrdId"	=>	$freezeOrdId[$k],
			"RetUrl"	=>	$retUrl,
			"BgRetUrl"	=>	$bgRetUrl,
			"MerPriv"	=>	$merPriv,
			"ReqExt"	=>	$reqExt,
			"ChkValue"	=>	$checkValue[$k],
			);
		}
		//echo "<pre>";print_r($reqData);echo "</pre>";exit;
		$chinapnrend = microtime(true);
		//Log::write("chinapnrtime_".strval($chinapnrend-$chinapnrstart));
		$num = floor(count($reqData)/10)+1;
		$requeststart = microtime(true);
		for($i=0;$i<$num;$i++)
		{
			$req = array_slice($reqData,10*$i,10);
			$this->request_multi($req);
		}
		$requestend = microtime(true);
		Log::write("requesttime_".strval($requestend-$requeststart));
	...
	{
		$divDetails = $this->arrayToJsonStr($divDetails);
		$LoansVocherAmt=M('active_redpacket')->field("sum(facevalue) as facevalue")->where('orderno='.$subOrdId)->find();
		$sql=M()->GetLastSql();
		file_put_contents('/tmp/debug2',date('m-d H:i:s')." newsql ".print_r($sql,true)."\n",FILE_APPEND);
		file_put_contents('/tmp/debug2',date('m-d H:i:s')." newLoansVocherAmt ".print_r($LoansVocherAmt,true)."\n",FILE_APPEND);
		if($LoansVocherAmt['facevalue'])
		{
		$LoansVocherAmt=number_format($LoansVocherAmt['facevalue'],2,'.','');//代金券
		file_put_contents('/tmp/debug2',date('m-d H:i:s')." newLoansVocherAmtlj ".print_r($LoansVocherAmt,true)."\n",FILE_APPEND);
		$reqExt=json_encode(array('LoansVocherAmt'=>$LoansVocherAmt));//每个标号是否使用了代金券
		file_put_contents('/tmp/debug2',date('m-d H:i:s')." newreqExt ".print_r($reqExt[$k],true)."\n",FILE_APPEND);
		}
		/*===结束=*/
		if($reqExt)
		{
		...
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_LOANS.$merCustId.$ordId.$ordDate.$outCustId.$transAmt.$fee.$subOrdId.$subOrdDate.$inCustId.$divDetails.$feeObjFlag.$isDefault.$isUnFreeze.$unFreezeOrdId.$freezeTrxId.$bgRetUrl.$merPriv.$reqExt);
		$reqData=array(
				"Version"	=>	Chinapnr::VERSION_20,
				"CmdId"		=>	Chinapnr::CMDID_LOANS,
				"MerCustId"	=>	$merCustId,
				"OrdId"	=>	$ordId,
				"OrdDate"	=>	$ordDate,
				"OutCustId"	=>	$outCustId,
		...
		file_put_contents('/tmp/debugs',date('m-d H:i:s')."\$reqData-newlj".print_r($reqData,true)."\n",FILE_APPEND);
	 ...
	 * @return Ambigous <multitype:, NULL, mixed>
	 */
	public function loans_multi($merCustId,$ordId,$ordDate,$outCustId,$transAmt,$fee,$subOrdId,$subOrdDate,$inCustId,$divDetails = '',$feeObjFlag,$isDefault,$isUnFreeze,$unFreezeOrdId = '',$freezeTrxId = '',$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		
		$checkValue=array();
		$reqData=array();		
		foreach($outCustId as $k=>$v)
		{
			$divDetails[$k] = $this->arrayToJsonStr($divDetails[$k]);
			//如果没有红包传入 不会报：json 转换字符串错误
			/*放款 也要传代金券*/
			$LoansVocherAmt=M('active_redpacket')->field("sum(facevalue) as facevalue")->where('orderno='.$subOrdId[$k])->select();
			$sql=M()->GetLastSql();
			file_put_contents('/tmp/debug2',date('m-d H:i:s')." sql ".print_r($sql,true)."\n",FILE_APPEND);
			file_put_contents('/tmp/debug2',date('m-d H:i:s')." LoansVocherAmt ".print_r($LoansVocherAmt,true)."\n",FILE_APPEND);
			if($LoansVocherAmt[0]['facevalue'])
			{
			$LoansVocherAmt=number_format($LoansVocherAmt[0]['facevalue'],2,'.','');//代金券
			file_put_contents('/tmp/debug2',date('m-d H:i:s')." LoansVocherAmt ".print_r($LoansVocherAmt,true)."\n",FILE_APPEND);
			$reqExt[$k]=json_encode(array('LoansVocherAmt'=>$LoansVocherAmt));//每个标号是否使用了代金券
			file_put_contents('/tmp/debug2',date('m-d H:i:s')." reqExt ".print_r($reqExt[$k],true)."\n",FILE_APPEND);
			}
			/*===结束=*/

			if($reqExt[$k])
			{
				$reqExt[$k]=$this->arrayToJsonStr($reqExt[$k]);
			}
	
			$checkValue[$k]= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_LOANS.$merCustId.$ordId[$k].$ordDate[$k].$outCustId[$k].$transAmt[$k].$fee[$k].$subOrdId[$k].$subOrdDate[$k].$inCustId.$divDetails[$k].$feeObjFlag.$isDefault.$isUnFreeze[$k].$unFreezeOrdId[$k].$freezeTrxId[$k].$bgRetUrl.$merPriv[$k].$reqExt[$k]);
			...
			$reqData[$k]=array(
					...
					"OrdId"	=>	$ordId[$k],
					"OrdDate"	=>	$ordDate[$k],
					"OutCustId"	=>	$outCustId[$k],
					"TransAmt"	=>	$transAmt[$k],
					"Fee"	=>	$fee[$k],
					"SubOrdId"	=>	$subOrdId[$k],
					"SubOrdDate"	=>	$subOrdDate[$k],
					"InCustId"	=>	$inCustId,
					"DivDetails"	=>	$divDetails[$k],
					"FeeObjFlag"	=>	$feeObjFlag,
					"IsDefault"	=>	$isDefault,
					"IsUnFreeze"	=>	$isUnFreeze[$k],
					"UnFreezeOrdId"	=>	$unFreezeOrdId[$k],
					"FreezeTrxId"	=>	$freezeTrxId[$k],
					"BgRetUrl"	=>	$bgRetUrl,
					"MerPriv"	=>	$merPriv[$k],
					"ReqExt"	=>	$reqExt[$k],
					"ChkValue"	=>	$checkValue[$k],
		...
		//echo "<pre>";print_r($reqData);echo "</pre>";exit;
		 file_put_contents('/tmp/debug2',date('m-d H:i:s')." reqData ".print_r($reqData,true)."\n",FILE_APPEND);
	...
	{
		$divDetails = $this->arrayToJsonStr($divDetails);
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_REPAYMENT.$merCustId.$ordId.$ordDate.$outCustId.$subOrdId.$subOrdDate.$outAcctId.$transAmt.$fee.$inCustId.$inAcctId.$divDetails.$feeObjFlag.$bgRetUrl.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_20,
			"CmdId"		=>	Chinapnr::CMDID_REPAYMENT,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"OrdDate"	=>	$ordDate,
			"OutCustId"	=>	$outCustId,
	 ...
	 * @return Ambigous <multitype:, NULL, mixed>
	 */
	public function repayment_multi($merCustId,$ordId,$ordDate,$outCustId,$subOrdId,$subOrdDate,$outAcctId = '',$transAmt,$fee,$inCustId,$inAcctId = '',$divDetails = '',$feeObjFlag,$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		$checkValue = array();
		$reqData = array();

				 file_put_contents('/tmp/debug',date('m-d H:i:s')." inCustId ".print_r($inCustId,true)."\n",FILE_APPEND);

		foreach($inCustId as $k=>$v)
		{
			$divDetails[$k] = $this->arrayToJsonStr($divDetails[$k]);
			$checkValue[$k]= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_REPAYMENT.$merCustId.$ordId[$k].$ordDate[$k].$outCustId.$subOrdId[$k].$subOrdDate[$k].$outAcctId.$transAmt[$k].$fee[$k].$inCustId[$k].$inAcctId.$divDetails[$k].$feeObjFlag.$bgRetUrl.$merPriv[$k].$reqExt);
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_TRANSFER.$ordId.$outCustId.$outAcctId.$transAmt.$inCustId.$inAcctId.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_TRANSFER,
			"OrdId"	=>	$ordId,
			"OutCustId"	=>	$outCustId,
			"OutAcctId"	=>	$outAcctId,
			"TransAmt"	=>	$transAmt,
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_CASH_AUDIT.$merCustId.$ordId.$usrCustId.$transAmt.$auditFlag.$retUrl.$bgRetUrl.$merPriv);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_CASH_AUDIT,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"UsrCustId"	=>	$usrCustId,
			"TransAmt"	=>	$transAmt,
	...
	public function merCash($merCustId,$ordId,$usrCustId,$transAmt,$servFee = '',$servFeeAcctId = '',$retUrl = '',$bgRetUrl,$remark = '',$charSet = '',$merPriv = '',$reqExt = '')
	{
		...
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_MER_CASH.$merCustId.$ordId.$usrCustId.$transAmt.$servFee.$servFeeAcctId.$retUrl.$bgRetUrl.$remark.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_20,
			"CmdId"		=>	Chinapnr::CMDID_MER_CASH,
			"MerCustId"	=>	$merCustId,
			"OrdId"	=>	$ordId,
			"UsrCustId"	=>	$usrCustId,
			"TransAmt"	=>	$transAmt,
	...
	public function autoCreditAssign($merCustId,$sellCustId,$creditAmt,$creditDealAmt,$bidDetails,$fee,$divDetails = '',$buyCustId,$ordId,$ordDate,$retUrl = '',$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		$bidDetails = $this->arrayToJsonStr($bidDetails);
		$divDetails = $this->arrayToJsonStr($divDetails);
		$checkValue= $this->sign_npc(Chinapnr::VERSION_10.Chinapnr::CMDID_AUTO_CREDIT_ASSIGN.$merCustId.$sellCustId.$creditAmt.$creditDealAmt.$bidDetails.$fee.$divDetails.$buyCustId.$ordId.$ordDate.$retUrl.$bgRetUrl.$merPriv.$reqExt);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_10,
			"CmdId"		=>	Chinapnr::CMDID_AUTO_CREDIT_ASSIGN,
			"MerCustId"	=>	$merCustId,
			"SellCustId"	=>	$sellCustId,
			"CreditAmt"	=>	$creditAmt,
			"CreditDealAmt"	=>	$creditDealAmt,
