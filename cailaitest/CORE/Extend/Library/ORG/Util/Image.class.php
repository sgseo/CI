...
class Image {
        ...
        if ($info !== false) {
            ...
            if ($type != 'gif' && function_exists('imagecreatetruecolor')){
                ...
                if($type == 'gif'){
	                ...
	                imagecolortransparent($thumbImg, $white);
                }
            }
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($length=4, $mode='1', $type='png', $width=100, $height=30, $verifyName='verify') {
        import('ORG.Util.String');
        $randval = String::randString($length, $mode);
        $fontPath=APP_PATH."/Common/fzcartoon.TTF";
        ...
        $backColor = imagecolorallocate($im,'255','255','255' );    //背景色（随机）$r[$key], $g[$key], $b[$key]
        $borderColor = imagecolorallocate($im,255,255,255);                    //边框色
        imagefilledrectangle($im, 0, 0, $width - 1, $height - 1, $backColor);
        imagerectangle($im, 0, 0, $width - 1, $height - 1, $borderColor);
        $stringColor = imagecolorallocate($im,'243','137','0');//mt_rand(0, 200), mt_rand(0, 120), mt_rand(0, 120) 修改为全橙色
        // 干扰
        for ($i = 0; $i < 10; $i++) {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            imagettftext($im, 16,  0 ,$i*14+20, $height/1.4, $stringColor, $fontPath,$randval{$i});
        }
    ...
    static function GBVerify($length=4, $type='png', $width=180, $height=50, $fontface='simhei.ttf', $verifyName='verify') {
        import('ORG.Util.String');
        $fontface=APP_PATH."/Common/fzcartoon.ttf";
