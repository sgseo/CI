...
class WithdrawlogAction extends ACommonAction
{
    ...
    */
    public function index()
    {
		...
		$count = M('member_withdrawlog w')->join("{$this->pre}members m ON w.uid=m.id")->where($map)->count('w.id');
		$p = new Page($count, C('ADMIN_PAGE_SIZE'));
		$page = $p->show();
		$Lsql = "{$p->firstRow},{$p->listRows}";
		//分页处理
		//$field= 'w.*,m.user_name,mi.real_name,w.id,w.uid ';
		//$list = M('member_withdrawlog w')->field($field)->join("{$this->pre}members m ON w.uid=m.id")->join("{$this->pre}member_info mi ON w.uid=mi.uid")->where($map)->order(' w.id DESC ')->limit($Lsql)->select();
