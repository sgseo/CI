...
function getAreaArticleList($parm) {
    ...
    foreach ($data as $key => $v) {
        ...
        //elseif(count($typefix)==1) $data[$key]['arturl'] = 
            ...
            $data[$key]['arturl'] = MU("Home/{$typeu}", "article", array("id" => "id-" . $v['id'], "suffix" => $suffix));
    }
    $row = array();
    $row['list'] = $data;
    $row['page'] = $page;
...
function getAreaTypeList($parm) {
    ...
    foreach ($data as $key => $v) {
        ...
        elseif ($v['parent_id'] == 0 && count($typefix) == 1)
            $data[$key]['turl'] = MU("Home/{$v['type_nid']}/index", "typelist", array("id" => $v['id'], "suffix" => $suffix));
        else
            $data[$key]['turl'] = MU("Home/{$typeu}/{$v['type_nid']}", "typelist", array("id" => $v['id'], "suffix" => $suffix));
    }
    $row = array();
    $row = $data;
...
function get_personal_benefit($uid) {
    ...
    $interest_collection = M('investor_detail')
            ...
            ->find();
    file_put_contents('/tmp/isnew2',date('m-d H:i:s')."\$interest_collection-:".print_r($interest_collection,true)."\n",FILE_APPEND);
    $transfer_interest_collection = M('transfer_investor_detail')
            ->field('sum(interest) as interest, sum(capital) as capital,sum(interest_fee) as fee')
            ->where("investor_uid={$uid} and status = 7")
            ->find();
 file_put_contents('/tmp/isnew2',date('m-d H:i:s')."\$利息啊-:".print_r($transfer_interest_collection,true)."\n",FILE_APPEND);
    ...
    $benefit['capital_collection'] = $interest_collection['capital'] + $transfer_interest_collection['capital']; // dai shou ben jin
     file_put_contents('/tmp/isnew2',date('m-d H:i:s')."\$利息！！！啊-:".print_r($benefit,true)."\n",FILE_APPEND);
    return $benefit;
}
...
function getIco($map) {
    ...
    if ($map['borrow_type'] == 2)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/d.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 3)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/m.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 4)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/jing.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 1)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/xin.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 5)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/ya.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 6)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/lbt.gif" align="absmiddle">';
    elseif ($map['borrow_type'] == 7)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/che.gif" align="absmiddle">';
    if ($map['repayment_type'] == 1)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/t.gif" align="absmiddle">';
    if (!empty($map['password']))
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/passw.gif" align="absmiddle">';
    if ($map['is_tuijian'] == 1)
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/tuijian.gif" align="absmiddle">';
    if ($map['reward_type'] > 0 && ($map['reward_num'] > 0 || $map['reward_money'] > 0))
        $str.='<img src="' . __ROOT__ . '/Style/H/images/icon/j.gif" align="absmiddle">';
    return $str . '&nbsp;&nbsp;';
}
...
function getVerify($uid) {
    ...
    $str = "";
    if ($vo['id_status'] == 1)
        $str.='&nbsp;<img alt="实名认证通过" src="' . __ROOT__ . '/Style/H/images/icon/id.gif"/>';
    else
        $str.='&nbsp;<img alt="实名认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/id_0.gif"/>';
    if ($vo['phone_status'] == 1)
        $str.='&nbsp;<img alt="手机认证通过" src="' . __ROOT__ . '/Style/H/images/icon/phone.gif"/>';
    else
        $str.='&nbsp;<img alt="手机认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/phone_0.gif"/>';
    if ($vo['email_status'] == 1)
        $str.='&nbsp;<img alt="邮件认证通过" src="' . __ROOT__ . '/Style/H/images/icon/email.gif"/></br>';
    else
        $str.='&nbsp;<img alt="邮件认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/email_0.gif"/></br>';
    if ($vo['user_leve'] != 0 && $vo['time_limit'] > time())
        $str.='&nbsp;<img alt="VIP会员" src="' . __ROOT__ . '/Style/H/images/icon/vip.gif"/></a>&nbsp;';
    else
        $str.='&nbsp;<img alt="不是VIP会员" src="' . __ROOT__ . '/Style/H/images/icon/vip_0.gif"/>';
...
function getVerify_ucenter($uid) {
    ...
    $str = "";
    if ($vo['id_status'] == 1)
        $str.='<a href="' . __APP__ . '/member/verify#fragment-3"><img alt="实名认证通过"   title="实名认证通过" src="' . __ROOT__ . '/Style/H/images/icon/id.gif"/></a>&nbsp;';
    else
        $str.='<a href="' . __APP__ . '/member/verify#fragment-3"><img alt="实名认证未通过"  title="实名认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/id_0.gif"/></a>&nbsp;';
    if ($vo['phone_status'] == 1)
        $str.='<a href="' . __APP__ . '/member/verify#fragment-2"><img alt="手机认证通过"   title="手机认证通过" src="' . __ROOT__ . '/Style/H/images/icon/phone.gif"/>&nbsp;';
    else
        $str.='<a href="' . __APP__ . '/member/verify#fragment-2"><img alt="手机认证未通过"   title="手机认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/phone_0.gif"/></a>&nbsp;';
    if ($vo['email_status'] == 1)
        $str.='<a href="' . __APP__ . '/member/verify?id=1#fragment-1"><img alt="邮件认证通过"   title="邮件认证通过" src="' . __ROOT__ . '/Style/H/images/icon/email.gif"/></a>&nbsp;';
    else
        $str.='<a href="' . __APP__ . '/member/verify?id=1#fragment-1"><img alt="邮件认证未通过"   title="邮件认证未通过" src="' . __ROOT__ . '/Style/H/images/icon/email_0.gif"/></a>&nbsp;';
...
//帐号资金变动 App Push  by zxm
function insert_push_information($data) {
    //member_id 和 content 任意为空，则不添加记录
    if(!empty($data['member_id']) || !empty($data['content'])) {
        $db_push = M('cailaipush.information', 'push_');
        $db_push->member_id = $data['member_id'];
        $db_push->title = "资金变动";
        $db_push->content = $data['content'];
        $db_push->range_type = '7'; //单播
        $db_push->flag = '0';
...
function getUserType($uid) {
    $res = M("members")->field("user_type")->where("id={$uid}")->find();
