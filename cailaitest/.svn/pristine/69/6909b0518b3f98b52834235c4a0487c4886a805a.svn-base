...
//                $endTime = 1430539199;
    ...
    //        exit();
     ...
     public function loan()
     {
       ...
       // 待还本金：
        // SELECT SUM(borrow_money) FROM `lzh_borrow_info` WHERE borrow_status=6 

//待还利息：
// SELECT SUM(interest) FROM `lzh_investor_detail`  WHERE   repayment_time = 0

//已还利息
// SELECT SUM(interest) FROM `lzh_investor_detail`  WHERE   repayment_time > 0 
        $bonus=M('borrow_info')->field('sum(borrow_money) as bm')->where('borrow_status=6')->select();
        // echo M()->getLastSql();
        $alreadyinterest=M('investor_detail')->field('sum(interest) as al')->where('repayment_time>0')->select();
        // echo M()->getLastSql();
        $noreadyinterest=M('investor_detail')->field('sum(interest) as no')->where('repayment_time=0')->select();
        // echo M()->getLastSql();
        // dump($bonus);
        // dump($alreadyinterest);
        // dump($noreadyinterest);
        $freeze=M('member_money')->field('sum(money_freeze) as total')->select();
        $this->bl=$bl;
        $this->bonus=$bonus;
        $this->alreadyinterest=$alreadyinterest;
        $this->noreadyinterest=$noreadyinterest;
        $this->freeze=$freeze;
        //会员统计
        $this->assign("search",$search);
        $this->assign('list',$row);
