...
//$menu_left[$i][$i."-".$j][] = array('客户端云推送',U('/admin/baidupush/'),1);
//$menu_left[$i][$i."-".$j][] = array('登陆接口管理',U('/admin/loginonline/'),1);
//$menu_left[$i][$i."-".$j][] = array("自动还款参数",U("/admin/auto/"),1);
$menu_left[$i][$i."-".$j][] = array("后台操作日志",U("/admin/global/adminlog"),1);
$menu_left[$i][$i."-".$j][] = array("短信发送日志",U("/admin/global/sendsmslog"),1);
$menu_left[$i][$i."-".$j][] = array("短信发送",U("/admin/global/sendsms"),1);
$menu_left[$i][$i."-".$j][] = array("余额提醒",U("/admin/global/alertsms"),1);
$j++;
...
$menu_left[$i][$i."-".$j][] = array('还款列表',U('/admin/borrow/repaylist'),1);

$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array("企业直投管理","#",1);
$menu_left[$i][$i."-".$j][] = array('添加企业直投',U('/admin/tborrow/add'),1);
$menu_left[$i][$i."-".$j][] = array('投资中借款',U('/admin/tborrow/underway'),1);
$menu_left[$i][$i."-".$j][] = array('待复审借款',U('/admin/tborrow/review'),1);
$menu_left[$i][$i."-".$j][] = array('复审未通过',U('/admin/tborrow/reject'),1);
$menu_left[$i][$i."-".$j][] = array('已流标借款',U('/admin/tborrow/turnoff'),1);
$menu_left[$i][$i."-".$j][] = array("还款中借款",U("/admin/tborrow/repayment"),1);
$menu_left[$i][$i."-".$j][] = array("已完成借款",U("/admin/tborrow/completed"),1);
...
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('商户还款管理','#',1);
$menu_left[$i][$i."-".$j][] = array('待还款列表',U('/admin/repayment/index'),1);

$i++;
$menu_left[$i]= array('会员管理','#',1);
$menu_left[$i]['low_title'][$i."-".$j] = array('会员管理','#',1);
$menu_left[$i][$i."-".$j][] = array('会员列表',U('/admin/members/index'),1);
$menu_left[$i][$i."-".$j][] = array('会员解冻资金',U('/admin/members/unfreeze'),1);
$menu_left[$i][$i."-".$j][] = array('企业会员列表',U('/admin/members/corp'),1);
$menu_left[$i][$i."-".$j][] = array('担保企业会员列表',U('/admin/members/danbao'),1);
$menu_left[$i][$i."-".$j][] = array('会员资料列表',U('/admin/members/info'),1);
$menu_left[$i][$i."-".$j][] = array('举报信息',U('/admin/jubao/index'),1);
$menu_left[$i][$i."-".$j][] = array('会员对账',U('/admin/memberaccount/duizhang'),1);
$menu_left[$i][$i."-".$j][] = array('会员推荐人补绑',U('/admin/memberaccount/memberrec'),1);

$menu_left[$i][$i."-".$j][] = array('超标记录',U('/admin/memberaccount/chaobiao'),1);
...
$menu_left[$i]['low_title'][$i."-".$j] = array('黑名单管理','#',1);
$menu_left[$i][$i."-".$j][] = array('添加黑名单',U('/admin/blacklist/add'),1);
$menu_left[$i][$i."-".$j][] = array('黑名单列表',U('/admin/blacklist/index'),1);
$j++;
...
$menu_left[$i][$i."-".$j][] = array('充值记录列表',U('/admin/Paylog/index'),1);
...
$menu_left[$i][$i."-".$j][] = array('提现记录列表',U('/admin/Withdrawlog/index'),1);
...
$menu_left[$i][$i."-".$j][] = array('财来网销售奖励',U('/admin/capitalall/zwindex'),1);
//2015-10-22
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('会员统计','#',1);
$menu_left[$i][$i."-".$j][] = array('会员数据统计',U('/admin/capitalall/test'),1);
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('资金进出统计','#',1);
$menu_left[$i][$i."-".$j][] = array('会员数据统计',U('/admin/capitalall/member'),1);
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('借款统计','#',1);
$menu_left[$i][$i."-".$j][] = array('借款数据统计',U('/admin/capitalall/loan'),1);
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('网站收益统计','#',1);
$menu_left[$i][$i."-".$j][] = array('网站收益数据统计',U('/admin/capitalall/cast'),1);
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('投资者收益统计','#',1);
$menu_left[$i][$i."-".$j][] = array('投资者收益统计',U('/admin/capitalall/red'),1);
//2015-10-22
$i++;
...
$j++;
$menu_left[$i]['low_title'][$i."-".$j] = array('托管账户管理','#',1);
$menu_left[$i][$i."-".$j][] = array('账户转账',U('/admin/tuoguan/zwtransfer'),1);
$menu_left[$i][$i."-".$j][] = array('转账给个人',U('/admin/tuoguan/zwtoperson'),1);
$menu_left[$i][$i."-".$j][] = array('托管账户设置',U('/admin/tuoguan/'),1);
$menu_left[$i][$i."-".$j][] = array('商户充值',U('/admin/tuoguan/pay'),1);
$menu_left[$i][$i."-".$j][] = array('商户取现',U('/admin/tuoguan/withdraw'),1);
$menu_left[$i][$i."-".$j][] = array('商户托管登录',U('/admin/tuoguan/login'),1);
$menu_left[$i][$i."-".$j][] = array('商户帐户信息',U('/admin/tuoguan/queryaccts'),1);
$menu_left[$i][$i."-".$j][] = array('交易状态查询',U('/admin/tuoguan/transstat'),1);
$menu_left[$i][$i."-".$j][] = array('商户扣款对账',U('/admin/tuoguan/trf'),1);
$menu_left[$i][$i."-".$j][] = array('投标对账',U('/admin/tuoguan/reconciliation'),1);
$menu_left[$i][$i."-".$j][] = array('取现对账',U('/admin/tuoguan/querycash'),1);
$menu_left[$i][$i."-".$j][] = array('充值对账',U('/admin/tuoguan/querysave'),1);
$menu_left[$i][$i."-".$j][] = array('企业开户状态查询',U('/admin/tuoguan/corpquery'),1);
