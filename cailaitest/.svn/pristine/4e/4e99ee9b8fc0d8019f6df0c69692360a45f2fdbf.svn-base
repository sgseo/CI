...
var T_month_min = 0;
var T_month_max = 0;
function Transfer(borrow_id,month,per){
	//var tnum = $("#tnum_"+borrow_id).val();
	//var total = parseInt(tnum)*per;
	...
	$("#money").val(money);
	//$("#transfer_invest_num").val(parseInt(tnum));
	$.ajax({
	  url: Transfer_invest_url+"/investcheck",
	  type: "post",
	  dataType: "json",
	  //data: {"tnum":tnum,"month":month,'borrow_id':borrow_id},
