...
//		alogsm("actwithdraw",0,1,session("u_id").$jsoncode);
	...
	*/
	public function netsave()
	{
	    ...
	    if (intval($arr['uid'])!=$this->uid){
	        $mess['uid']=$arr['uid'];
	        $mess['thisuid']=$this->uid;
	        //$mess['TransAmt']=$arr['transamt'];
	        ajaxmsg($mess);
			//ajaxmsg("用户错误！",0);
	       }
	       	$transamt=$arr['transamt'];
	       	$ord = $this->ord();
		    $info = M("huifulog")->field("usrid")->where("uid=".intval(session("u_id")))->find();
			$usrid = $info['usrid'];
			$mess['Version']	=Chinapnr::VERSION_10;
			$mess['CmdId']		=Chinapnr::CMDID_NET_SAVE;
			$mess['MerCustId']	=$this->merCustId;
			$mess['UsrCustId']	=$usrid;
			$mess['OrdId']	=	$ord['id'];
			$mess['OrdDate']	=$ord['date'];;
			$mess['GateBusiId']	='';
			$mess['OpenBankId']	='';
			$mess['DcFlag']	='';
			$mess['TransAmt']=number_format($transamt,2,'.','');
			$mess['RetUrl']	= '';
			$mess['BgRetUrl']=	Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::NET_SAVE_BACK;
			$mess['MerPriv']='';
           // $checkValue=Chinapnr::sign($mess['Version'].$mess['CmdId'].$mess['MerCustId'].$mess['UsrCustId'].$mess['OrdId'].$mess['OrdDate'].$mess['GateBusiId'].$mess['OpenBankId'].$mess['DcFlag'].$mess['TransAmt'].$mess['RetUrl'].$mess['BgRetUrl'].$mess['MerPriv']);
			$mess['ChkValue']	='';

		ajaxmsg($mess);
	}
	/*public function netsave_url()
	{
		$mess = array();
		$jsoncode = file_get_contents("php://input");
		$arr = array();
		$arr = json_decode($jsoncode,true);
	    if (intval($arr['uid'])!=intval(session("u_id"))){
	        $mess['uid']=$arr['uid'];
	        $mess['thisuid']=intval(session("u_id"));
	        //$mess['TransAmt']=$arr['transamt'];
	        ajaxmsg($mess);
			//ajaxmsg("用户错误！",0);
	       }
	...
	*/
	public function cash()
	{
			...
			$transamt=$arr['transamt'];
			$ord = $this->ord();
			$mess['Version']=Chinapnr::VERSION_20;
			$mess['CmdId']=Chinapnr::CMDID_CASH;
			$mess['MerCustId']=	$this->merCustId;
			$mess['OrdId']=$ord['id'];
			$mess['UsrCustId']=$usrid;
			$mess['TransAmt']=number_format($transamt,2,'.','');
			...
			$mess['OpenAcctId']	='';
			$mess['RetUrl']	='';
			$mess['BgRetUrl']=Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::CASH_BACK;
			$mess['Remark']='';
			$mess['CharSet']='';
			$mess['MerPriv']='';
			$mess['ReqExt']='';
	...
	public function cash_url()
	{
			...
			$mess['url']="http://mertest.chinapnr.com/muser/publicRequests".
						  "&Version=".$arr['Version'].
						  "&CmdId=".$arr['CmdId'].
						  "&MerCustId=".$arr['MerCustId'].
						  "&OrdId=".$arr['OrdId'].
						  "&UsrCustId=".$arr['UsrCustId'].
						  "&TransAmt=".$arr['TransAmt'].
						  "&ServFee=".$arr['ServFee'].
						  "&ServFeeAcctId=".$arr['ServFeeAcctId'].
						  "&OpenAcctId=".$arr['OpenAcctId'].			  
						  "&RetUrl=".$arr['RetUrl'].
						  "&BgRetUrl=".$arr['BgRetUrl'].
						  "&Remark=".$arr['Remark'].
						  "&CharSet=".$arr['CharSet'].
						  "&MerPriv=".$arr['MerPriv'].
						  "&ReqExt=".$arr['ReqExt'].
						  "&ChkValue=".$arr['ChkValue'];
			ajaxmsg($mess);
	...
	*/
	public function usrFreezeBg()
	{
		...
		$usrid = $info['usrid'];
		$transamt=$arr['transamt'];
		$mess['Versio']	=	Chinapnr::VERSION_10;
		$mess['CmdI']		=	Chinapnr::CMDID_USR_FREEZE_BG;
		$mess['MerCustI']	=	$this->merCustId;
		$mess['UsrCustId']	=	$usrid;
		$mess['SubAcctType']	=	'';
		$mess['SubAcctId']	=	'';
		$mess['OrdId']	=	$ord['id'];
		...
		$mess['TransAmt']	=	 number_format($transamt,2,'.','');
		$mess['RetUrl']	=	'';
		$mess['BgRetUrl']	=  Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_FREEZE_BACK;
		$mess['MerPriv']	=	'';
		$mess['ChkValue']	=	'';
		ajaxmsg($mess);
	}
	...
	**$trxid 交易流水号
	*/
	// public function usrUnFreeze($trxid)
	// {
	// 	$mess = array();
	//     $jsoncode = file_get_contents("php://input");
	// 	$arr = array();
	...
	// 	$trxId = $trxid;
	// 	$retUrl = Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_UNFREEZE;
	// 	$bgRetUrl = Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_UNFREEZE_BACK;
		
	// 	$mess['Version']=	Chinapnr::VERSION_10;
	// 	$mess['CmdId']	=	Chinapnr::CMDID_USR_UN_FREEZE;
	// 	$mess['MerCustId']=	$this->merCustId;;
	// 	$mess['OrdId']=	$ord['id'];
	// 	$mess['OrdDate']=	$ord['date'];
	// 	$mess['TrxId']=	$trxId;
	// 	$mess['RetUrl']=	'';
	// 	$mess['BgRetUrl']=	Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::USER_UNFREEZE_BACK;
	// 	$mess['MerPriv']=	'';
	// 	$mess['ChkValue']=	'';
	// 	ajaxmsg($mess);
	// }
	...
	*/
	...
	{
        //$usrid,$transamt,$borrowercustid,$type,$borrowid
		...
		$type=$arr['type'];
		$mess['Version']=Chinapnr::VERSION_20;
		$mess['CmdId']=Chinapnr::CMDID_INITIATIVE_TENDER;
		$mess['MerCustId']=$this->merCustId;
		$mess['OrdId']=$ord['id'];
		$mess['OrdDate']=$ord['date'];
		$mess['TransAmt']=number_format($transamt,2,'.','');
		$mess['UsrCustId']=$usrid;
		$mess['MaxTenderRate']= $glo['maxtenderrate'];//最大投资手续费率
			// $borrowerDetails = array();//借款人信息
			// $borrowerDetails['BorrowerCustId'] = $borrowercustid;
			// $borrowerDetails['BorrowerAmt'] = number_format($transamt,2,'.','');
			// $borrowerDetails['BorrowerRate'] =$glo['borrowerrate'];//借款手续费率
		$mess['BorrowerCustId']=$borrowercustid;
		$mess['BorrowerAmt']=number_format($transamt,2,'.','');
		$mess['BorrowerRate']=$glo['borrowerrate'];//借款手续费率
		//$mess['BorrowerDetails'] = $this->arrayToJsonStr($borrowerDetails);
		if(strcmp($type,"S")==0)
		{
			$isFreeze = "Y";//是否冻结
			$freezeord = $this->ord();
		...
		$mess['IsFreeze']=$isFreeze;
		$mess['FreezeOrdId']=$freezeOrdId;
		$mess['RetUrl']	='';
		$mess['BgRetUrl']= Huifu::HTTP.$_SERVER["SERVER_NAME"].Huifu::INI_TENDER_BACK;
		$info = array();
			$info['type'] =$type;
			$info['borrowerid'] = $borrowercustid;
			$info['borrowid'] = $borrow_id;
		$mess['MerPriv']=base64_encode(json_encode($info));
		$mess['ReqExt']	='';
		$mess['ChkValue']='';
	...
	//主动投标地址返回
	public function initiativeTender_url()
	{
		$mess = array();
			$jsoncode = file_get_contents("php://input");
			$arr = array();
			...
			$mess['url']="http://mertest.chinapnr.com/muser/publicRequests".
						 "&Version=".$arr['Version'].
						  ...
						  "&TransAmt=".$arr['TransAmt'].
						  "&UsrCustId=".$arr['UsrCustId'].
						  "&MaxTenderRate=".$arr['MaxTenderRate'].
						  "&BorrowerDetails=".$arr['BorrowerDetails'].	
						  "&IsFreeze=".$arr['IsFreeze'].			  
						  "&FreezeOrdId=".$arr['FreezeOrdId'].			  
						  "&RetUrl=".$arr['RetUrl'].
						  "&BgRetUrl=".$arr['BgRetUrl'].
						  "&MerPriv=".$arr['MerPriv'].
						  "&ReqExt=".$arr['ReqExt'].
