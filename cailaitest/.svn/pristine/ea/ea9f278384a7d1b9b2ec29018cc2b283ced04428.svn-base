...
class InvestAction extends HCommonAction {
 ...
 public function sess()
 {
	...
	public function investcheck(){
        ...
        //是否使用了红包 money 是 投资金额 $amoney 是账户余额
        $redvalue=$_POST['redvalue'];
		if($money>$amoney){
            if($redvalue)//是否使用了红包
            {
                $bmoney=$redvalue+$amoney;//账户余额+所选红包总额
                if($money>$bmoney)//如果投资的钱>余额和所选红包
                {
    			$msg = "尊敬的{$uname}，您准备投标{$money}元，但您的账户可用余额与所选红包总额为{$bmoney}不足以支付此次投标，您要先去充值吗？";
    			ajaxmsg($msg,2);
                }else{
        ...
        if($_GET['borrow_id']){
           ...
           foreach($list as $k=>$v){
    		...
    		public function redpacket()
    		{
    			...
    			if(!$this->uid){ajaxmsg("请先登陆",0);};
    			$time=time();
    		$redlist=M("active_redpacket")->where('owner='.$uid.' and '.$time." < overtime and is_success='0'")->order('addtime asc,shelftime asc')->select();
            ...
            public function newbie()
            {
                ...
                $record=M('newbie_record')->count('id');

                $this->zong=$record;

                $rtotal=M("newbie_record")->field("sum(invest_money) as total")->select();

              //  $this->newinvestRecord($id);
             ...
             public function newinvestRecord($id=0)
            {
                ...
                if($_GET['id']){
                    $list = M("newbie_record as b")
                                ->field('b.username,b.invest_money,b.invest_time')
                                ->where('b.status=7')->order('b.invest_time desc')
            ...
            public function newbie_check()
            {
                ...
                $m = M("member_money")->field('account_money,back_money,money_collect')->find($this->uid);
                $amoney = $m['account_money']+$m['back_money'];
                $uname = session('u_user_name');//用户名称
            ...
            */
            public function newbie_repayment()
            {
               ...
               $cron= $huifu->repayment($usrid,$transamt,$Fee,$investor,$divdetails,$subordid,$suborddate,$borrowid,$sort_order='',$repaymentid,$investorid,$type);//还款  
               if($investid){
                echo "{$investid}还款成功";
                }
            }
