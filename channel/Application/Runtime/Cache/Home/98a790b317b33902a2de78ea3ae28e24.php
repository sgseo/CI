...
<body>
<form class="form-inline definewidth m20" action="/channel/Home/Role/index" method="post">  
    我推荐的人员列表&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    推荐者手机：
    ...
    <a class="btn btn-success" href="/channel/Home/Role/cl_to_excel">下载</a>
</form>
<table class="table table-bordered table-hover definewidth m10" >
    ...
    <?php if(is_array($result)): $i = 0; $__LIST__ = array_slice($result,$cls,$spc,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
          ...
          <td>
            <a href="/channel/Home/Role/rectendback?userId=<?php echo ($vo["id"]); ?>&real_name=<?php echo ($vo["real_name"]); ?>">投资记录</a>&there4;
            <a href="/channel/Home/Role/remember?userId=<?php echo ($vo["id"]); ?>&real_name=<?php echo ($vo["real_name"]); ?>&phone=<?php echo ($vo["mobile"]); ?>">资金状况</a></td>
            <!-- <?php echo ($vo["mobile"]); ?> -->
    ...
    <div class='table table-bordered table-hover definewidth m10' style='margin:0 auto;text-align:center;'>
           <?php for($i=1;$i<=$total;$i++) { echo "<a href='/channel/Home/Role/?p=$i&tp=$tp'>"."第".$i.'页</a>&nbsp;&nbsp;'; } $p=$p?$p:1;$total=$total?$total:1; echo "当前第<span style='color:red'>&nbsp;".$p.'&nbsp;</span>页'."共".$total."页"; ?>
    </div>
...
<?php if(count($tinvestors) != 0): ?>ThinkPHP
    ...
    <div class='table table-bordered table-hover definewidth m10' style='margin:0 auto;text-align:center;'>
           <?php for($i=1;$i<=$ttotal;$i++) { echo "<a href='/channel/Home/Role/index?&p=$p&tp=$i'>"."第".$i.'页</a>&nbsp;&nbsp;'; } $tp=$tp?$tp:1;$ttotal=$ttotal?$ttotal:1; echo "当前第<span style='color:red'>&nbsp;".$tp.'&nbsp;</span>页'."共".$ttotal."页"; ?>
    </div>
