...
/*************    方法     **************/
	...
	function kalendarCode(codeYear,codeMonth,codeDay,codeFirst){
		...
		// 	}
		// }
		kalendar_html +="<h2 style='text-align:center;'>"+codeYear+'年';
		kalendar_html +=(codeMonth+1)+'月'+"</h2>";
...
// \n<div id='time'>\n</div>
		...
		for(var m=0;m<6;m++){//日期共 4-6 行
			...
			for(var n=0;n<7;n++){//列
				if((7*m+n) < codeFirst || (7*m+n) >= (codeFirst+monthDays[codeMonth])){//某月日历中不存在的日期
					...
					if((7*m+n+1-codeFirst == codeDay)&&(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6))){//当天是末
						kalendar_html += "<li class='todayWeekend' id='"+(7*m+n+1-codeFirst)+"'>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(((7*m+n)%7 == 0) || ((7*m+n)%7 == 6)){//仅是末
						kalendar_html += "<li class='weekend' id='"+(7*m+n+1-codeFirst)+"'>"+(7*m+n+1-codeFirst)+"</li>";
					}else if(7*m+n+1-codeFirst == codeDay){//仅是当天
						kalendar_html += "<li class='today' id='"+(7*m+n+1-codeFirst)+"'>"+(7*m+n+1-codeFirst)+"</li>";
					}else{//其他日期
						kalendar_html += "<li id='"+(7*m+n+1-codeFirst)+"'>"+(7*m+n+1-codeFirst)+"</li>";
					}
				}
			}
			kalendar_html += "</ul>\n";
	...
	//修改日历列表 的方法
	//参数依次为 操作对象(每一天) 月份 修改后的第一天是几 修改后的总天数 当天的具体日期
...
/*************   将日历代码放入相应位置，初始时显示此处内容      **************/
	...
	// alert('年:'+someDay['year']+'\n月:'+someDay['month']+'\n日:'+someDay['date']+'\n:'+someDay['week']+'\n本月第一天:'+someDay['firstDay']);return false;
