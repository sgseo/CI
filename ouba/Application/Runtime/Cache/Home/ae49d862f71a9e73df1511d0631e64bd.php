...
<div class="sidebar-s"> 
 ...
 <!--    <div class='pic'> -->
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["class_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
    </select>
...
<!-- 图片展示 -->
     <div class='pic_strap definewidth m10 table-bordered' style='border:1px solid #ccc;width:100%;height:<?php echo ($pheight); ?>px;'>
        ...
        <?php if(is_array($plist)): $k = 0; $__LIST__ = $plist;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class='ims' style=' float:left;width:265px;height:202px;border:1px solid #999;margin:20px 3% 0px 3%;'>
                <?php if(($vo["file_type"]) != "2"): ?><span><?php echo ($vo["class_name"]); if(($$vo["newpictype"]) != "0"): echo ($vo["newclass"]); endif; ?></span>
                   <img src="/Uploads/<?php echo ($vo["path"]); ?>" width='170px' class='scale' title='点击图片进行放大'/><?php endif; ?>
...
<div class="sidebar-frame"> 
  <div class="sidebar-s"> 
    <!-- <input  type='button' class="btn btn-large btn-primary btn btn-default" id='new' style='margin:10px 10px;'value='添加新类别'/> -->
      <a href='javascript:void(0);' class="btn btn-large btn-primary btn btn-default"  id='end' >完成</a>
    </div>
</div>
</form>
</div> 
...
<script type='text/javascript'>
   ...
   function bilibili(a)
   {
    ...
    $("#end").click(function(){
    window.location.href='/index.php/home/role/testsuc';
    })
</script>