...
<!-- 项目录入 -->
<form  class="table table-hover definewidth m10" style="padding-top:20px;" action="/index.php/Home/Menu/likeindex" method="post" enctype="multipart/form-data" >
<!-- 基础文档模型 -->
...
<div class='every_p'>
<span>抵押人姓名: </span>
<input type="text" class="width_353" value='<?php echo ($returndata["pledgename"]); ?>' name='pledgename' />
</div>
<div class='every_p'>
<span>抵押人电话: </span>
<input type="text" class="width_353" value='<?php echo ($returndata["pledgetel"]); ?>' name='pledgetel' />
</div>
<div class='every_p'>
<span>抵押人身份证号码: </span>
<input type="text" class="width_338" value='<?php echo ($returndata["pledgeidno"]); ?>' name='pledgeidno' />
</div>
</div>              
<!-- 抵押人信息 -->
...
<div class="every_p">
<div class='son_p'>
<p><input  type="radio" value="1" name="iscity"  <?php if(($returndata["iscity"]) == "1"): ?>checked=checked<?php endif; ?>/>主城区</p>
<p><input  type="radio"  name="iscity" value='2' <?php if(($returndata["iscity"]) == "2"): ?>checked=checked<?php endif; ?>/>非主城区</p>
...
<option value="0">请选择</option>
<?php if(is_array($htlist)): $i = 0; $__LIST__ = $htlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if(($returndata["housetype"]) == $vo["sort"]): ?>selected='selected'<?php endif; ?>><?php echo ($vo["housetype"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
</select>
</div>
</span>
<div class="clear"></div>
...
<textarea name="tip" placeholder="备注信息" rows='5' cols='100'><?php echo ($returndata["tip"]); ?></textarea>
</div>
