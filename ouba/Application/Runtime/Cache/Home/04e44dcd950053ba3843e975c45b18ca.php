...
function showChineseAmount(v,i) {
 ...
 <!--    <a href='/index.php/Home/Role/allotlist?status=4' class="btn btn-primary">已分配</a>
    <a href='/index.php/Home/Role/allotlist?status=2' class="btn btn-primary">待分配</a>
    <a href='/index.php/Home/Role/allotlist?status=3' class="btn btn-primary">已拒绝</a> -->
<form class="form-inline definewidth m20" action="/index.php/Home/Role/mytask" method="post">  
    客户姓名：
...
<!--         <th>房屋地址</th>
    ...
    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
        ...
        <!--       <td><?php if($vo["address"] != null): echo ($vo["address"]); else: ?>-<?php endif; ?></td>
            ...
            <td><?php if($vo["residence"] != 0): echo ($vo["residence"]); else: ?>-<?php endif; ?></td>
            ...
            <td><?php if(($vo["category"]) == "car"): ?>车抵押&nbsp;<input type="button" value="&nabla;" onclick="demo08(<?php echo ($vo["id"]); ?>);" /><?php endif; if(($vo["category"]) == "fang"): ?>房抵押&nbsp;<input type="button" value="&nabla;" onclick="demo08(<?php echo ($vo["id"]); ?>);" /><?php endif; ?></td>
            <td><?php if($vo["fptime"] != 0): echo (date(Ymd,$vo["fptime"])); else: ?>-<?php endif; ?></td>                      

            <td>
                <?php echo ($vo["status"]); ?>
            <!--     <?php if($vo['status']==2) { echo '待分配'; }else if($vo['status']==3){ echo '已拒绝'; }else if($vo['status']==4) { echo "已分配"; }else if($vo['status']==5) { echo "已处理"; }else if($vo['status']==6) { echo "已放款"; } ?> -->
            </td>
            <td><?php if($vo["status"] != 5): ?><span class='confirm'><a href='/index.php/Home/Role/taskconfirm?taskid=<?php echo ($vo["id"]); ?>&uid=<?php echo ($uid); ?>'>已处理</a></span><?php else: ?>-<?php endif; ?></td>
