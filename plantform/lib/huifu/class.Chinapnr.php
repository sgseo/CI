<?php 
include_once dirname(__FILE__)."/class.SecureTool.php";
...
class Chinapnr {
	 ...
	 * @param string $merId
	 */
	public function Chinapnr(){
		
		require_once('config/config.huifu.php');
		$this->merId		= _HUIFU_MERCUSTID ;
		$this->platformUrl	= _HUIFU_URL;
		$this->scureTool	= new SecureTool(_HUIFU_PRIVATEKEY,_HUIFU_PUBLICKKEY);
		file_put_contents('/tmp/chinahuifu',date('m-d H:i:s')." scureTool : ".print_r($this->scureTool,true)."\n",FILE_APPEND);
    ...
    private function sign_npc($msg)
    {
    	...
    	$npc_server_host = _NPC_HOST;
    	$npc_server_port = _NPC_PORT;
    	$npc_server_type_sign = _NPC_SIGN;
    	$npc_server_type_ver = _NPC_VER;
    	$npc_server_id = _NPC_ID;
        /*
	 ...
	 */
	public function reactResponse($res= "", $signKeys=array()){
	...
	public function verify($originStr, $sign){
		file_put_contents('/tmp/chinahuifu',date('m-d H:i:s')." originStr : ".print_r($originStr,true)."\n",FILE_APPEND);
		file_put_contents('/tmp/chinahuifu',date('m-d H:i:s')." sign : ".print_r($sign,true)."\n",FILE_APPEND);
...
HTML;
		...
		file_put_contents('autoRedirect.txt',date('m-d H:i:s')."autoRedirect ".print_r($tmp,true)."\n",FILE_APPEND);
		print $html;
		exit;
	}
...
//                file_put_contents('/tmp/zwerror',date('m-d H:i:s')."autoTenderPlan".print_r($reqData,true)."\n",FILE_APPEND);
	...
	public function netSave($merCustId,$usrCustId,$ordId,$ordDate,$gateBusiId = '',$openBankId = '',$dcFlag = '',$transAmt,$retUrl = '',$bgRetUrl,$merPriv = '')
	{
		...
		//file_put_contents('/tmp/netSave',date('m-d H:i:s')." netSave : ".print_r($reqData,true)."\n",FILE_APPEND);
	...
	{
	file_put_contents('/tmp/zw2.txt',date('m-d H:i:s')."\$transamt-pnr ".print_r($transAmt,true)."\n",FILE_APPEND);
		$borrowerDetails = $this->arrayToJsonStr($borrowerDetails);
		//$reqExt = $this->arrayToJsonStr($reqExt);
		//$reqExt=array("Vocher"=>array("AcctId"=>'MDT000001',"VocherAmt"=>'30.00'));
		if($reqExt)
		{
		//$reqExt=json_encode($reqExt);
		//$reqExt = $this->arrayToJsonStr($reqExt);//json 字符串
		$reqExt = $this->arrayToJsonStr2($reqExt);
		}
		...
		//{"Vocher":{"AcctId":"MDT000001","VocherAmt":"20.00"}}

         return $reqData;//传给API 然后给APP
    }
	/**
	...
	{
		$checkValue= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_CASH.$merCustId.$ordId.$usrCustId.$transAmt.$servFee.$servFeeAcctId.$openAcctId.$retUrl.$bgRetURL.$remark.$merPriv.$reqExt.$cashChl);
		$reqData=array(
			"Version"	=>	Chinapnr::VERSION_20,
	...
	public function loans_multi($merCustId,$ordId,$ordDate,$outCustId,$transAmt,$fee,$subOrdId,$subOrdDate,$inCustId,$divDetails = '',$feeObjFlag,$isDefault,$isUnFreeze,$unFreezeOrdId = '',$freezeTrxId = '',$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		...
		foreach($outCustId as $k=>$v)
		{
			...
			$reqExt = $this->arrayToJsonStr($reqExt);
			$checkValue[$k]= $this->sign_npc(Chinapnr::VERSION_20.Chinapnr::CMDID_LOANS.$merCustId.$ordId[$k].$ordDate[$k].$outCustId[$k].$transAmt[$k].$fee[$k].$subOrdId[$k].$subOrdDate[$k].$inCustId.$divDetails[$k].$feeObjFlag.$isDefault.$isUnFreeze[$k].$unFreezeOrdId[$k].$freezeTrxId[$k].$bgRetUrl.$merPriv[$k].$reqExt);
	 ...
	 * @return string
	 */
	private function arrayToJsonStr2($array = '')
	{
		if (is_array($array))
		{
			return json_encode($array);
