...
class SecureTool{
	...
	private function buildKey($key){
		...
		$enc = $this->like_mcrypt_cbc ( $cipher, SecureTool::DES_KEY, $prime1, MCRYPT_DECRYPT, $iv );
		$private_key ["prime1"] = $enc;
		$prime2 = substr ( $bin, 448, 64 );
		$enc = $this->like_mcrypt_cbc ( $cipher, SecureTool::DES_KEY, $prime2, MCRYPT_DECRYPT, $iv );
		$private_key ["prime2"] = $enc;
		$prime_exponent1 = substr ( $bin, 512, 64 );
		$enc = $this->like_mcrypt_cbc ( $cipher, SecureTool::DES_KEY, $prime_exponent1, MCRYPT_DECRYPT, $iv );
		$private_key ["prime_exponent1"] = $enc;
		$prime_exponent2 = substr ( $bin, 576, 64 );
		$enc = $this->like_mcrypt_cbc ( $cipher, SecureTool::DES_KEY, $prime_exponent2, MCRYPT_DECRYPT, $iv );
		$private_key ["prime_exponent2"] = $enc;
		$coefficient = substr ( $bin, 640, 64 );
		$enc = $this->like_mcrypt_cbc ( $cipher, SecureTool::DES_KEY, $coefficient, MCRYPT_DECRYPT, $iv );
	...
	private function secure_tool_sha1_128($string) {
		...
		$sha_pad = $this->secure_tool_hex2bin ( SecureTool::HASH_PAD );
