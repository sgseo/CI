...
//获得黑名单
    ...
    public function getUserRecommend($userId){
        $db = core::db()->getConnect('CAILAI', true);       
        $sql = sprintf("SELECT m.id,m.user_name AS mobile,m.reg_time AS reg_date,sum(mm.affect_money) bonus,mi.real_name FROM %s AS m LEFT JOIN %s AS mm ON (m.id=mm.target_uid) LEFT JOIN %s AS mi ON(mi.uid=m.id) WHERE (m.recommend_id ='%s') GROUP BY  m.id",$this->table,$this->tablem,$this->tablei,$userId);
...
//                
	...
	public function get_capital($user_id) {
		...
		$data['account_money'] = floatval($capital['account_money']+$capital['back_money']);//帐户余额
		$data['back_money'] = (float)$capital['back_money'];//累计收益
		$data['all_money'] = floatval($capital['account_money']+$capital['back_money']+$capital['money_collect']+$capital['freeze_money']);//总资产
		$data['money_freeze'] = (float)$capital['money_freeze'];//冻结金额
		$data['money_collect'] = (float)$capital['money_collect'];//待收总额
		$data['wait_interest'] = (float)$interst;//待收利息,还没算出来
		return $data;
	}
